            ...
            .methods(
                    {
                        ...
                        _moveAttrs : function(element, elementConfig, product) {
                            if (elementConfig.attrs && !!product) {
                                this._keepAttrsOrder(element, elementConfig);
                        ...
                        // LEOS-4410 reorder attributes to be handled in the same order as the 'from' element.
                        // Transformation will, then, not change the order of the attributes
                        _keepAttrsOrder : function _keepAttrsOrder(element, elementConfig) {
                            if (elementConfig.attrs && element.attributes) {
                                var counter = 0;
                                for (var key in element.attributes) {
                                    var index = elementConfig.attrs.map(function (attr) {
                                        return attr.from;
                                    }).indexOf(key);
                                    if (index != -1) {
                                        if (counter != index) {
                                            var tmp = elementConfig.attrs[counter];
                                            elementConfig.attrs[counter] = elementConfig.attrs[index];
                                            elementConfig.attrs[index] = tmp;
                                        }
                                        counter++;
                                    }
                                }
                        ...
                        _moveValueForTextNode : function _moveValueForTextNode(element, elementConfig, product) {
                            ...
                            if (elementConfig.to === "text" || element.transformationContext.elementName === "text") {
                                ...
                                } else if(elementConfig.fromAttribute){
                                    elementValue = element.attributes[elementConfig.fromAttribute.toLowerCase()];
                                } else if(element.children[0] && element.children[0].type === CKEDITOR.NODE_TEXT){
                                    elementValue = element.children[0].value;
                                }

                                if (elementConfig.to === "text") {
                                    if (elementConfig.fromAttribute && !elementValue) {
