...
define(function leosHierarchicalElementSubparagraphAfterLastPointModule(require) {
    ...
    var AKN_ANNEX_ORDERED_LIST = "aknAnnexOrderedList";
    ...
    var isShiftCtrlEnterAllowedInThisContext = function isShiftCtrlEnterAllowedInThisContext(editor) {
        ...
        currentElement = (currentElement != null) ? currentElement.getAscendant("li", true) : undefined;        
        if (!currentElement) {
            return false;
        }
        ...
        switch (editor.LEOS.elementType) {
            case LEVEL_ELEMENT_TYPE:
                if (!isElementInsideList(currentElement, LEVEL_ELEMENT_TYPE, AKN_ANNEX_ORDERED_LIST)
    ...
    function isLastElementInsideList(el, elementType) {
        do {
            ...
            el = el.getAscendant("li")
                ...
                || (elementType === LEVEL_ELEMENT_TYPE && el.getAscendant('ol') && el.getAscendant("ol").getAttribute(DATA_AKN_NAME) != AKN_ANNEX_LIST)));
