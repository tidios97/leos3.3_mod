 ...
 *     - Added event and function 'refreshToolbar'.
 *     - getToolbarHtml function is the content of previous uiSpace event function; getToolbarHtml is called by 'uiSpace' and 'refreshToolbar' listeners
 *     - All changes are commented with 'LEOS-2471' reference.
 */
/**
...
; // jshint ignore:line
define(function leosToolbarPluginModule(require) {
    "use strict";
    ...
    var pluginDefinition = {
        ...
        init: function( editor ) {
            editor.lang.toolbar = editor.lang.leosToolbar; //LEOS-2471 Assign language toolbar config to the normal toolbar object
            var endFlag;
...
//LEOS-2471 Customization: start
//LEOS-2471 Added event and function 'refreshToolbar'
            editor.on( 'refreshToolbar', refreshToolbar);
            function refreshToolbar( event ) {
                var e = new CKEDITOR.event();
                e.data = {
                    space: editor.config.toolbarLocation,
                    html: ''
                };

                var obj = getToolbarHtml(e);

                // Replace the toolbar HTML 
                var tbEleId = editor.id +"_" + editor.config.toolbarLocation;
                var tbEle = document.getElementById(tbEleId);
                $(tbEle).html(e.data.html);
            };

            editor.on( 'uiSpace', getToolbarHtml);
            function getToolbarHtml( event ) {
//LEOS-2471 Customization: end
