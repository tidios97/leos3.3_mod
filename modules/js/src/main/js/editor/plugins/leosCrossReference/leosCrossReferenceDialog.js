...
define(function leosCrossReferenceDialog(require) {
                        ...
                        var elements = [
                            {
                                ...
                                setup: function setup(widget) {
                                    ...
                                    var children = $(widget.element.$).children('ref').filter(function () {
                                        ...
                                        if (hrefArr.length >= 2) {
                                            var documentRef = hrefArr[0];
                                            return documentRef === documentMetadata.ref;
                                        } else {
                                            return editor.LEOS.documentRef === documentMetadata.ref;
                                        }
                                    });
    ...
    /**
     * For some unidentified reason, the styles are applied at a later time after the dialog window is displayed
     * and because of that it has its dimensions changed.
     * Because of this, it was necessary to re-center the window using a timer.
     */
    ...
    var nodeContentHandler = {
        ...
        requestRefLabel: function requestRefLabel(nodeList) {
            ...
            if (range.startOffset == 0) {
                ...
                var CHAR_NBSP = 160;
                var textInEditor;
                var indexPositionBefore;
                if (range.startContainer.$.nodeType === Node.ELEMENT_NODE) {
                    textInEditor = Util.getLastTextNodeUpTo(range.startContainer.$).textContent;
                    indexPositionBefore = textInEditor.length - 1;
                } else if (range.startContainer.$.nodeType === Node.TEXT_NODE) {
                    textInEditor =  range.startContainer.$.data;
                    indexPositionBefore = range.startOffset - 1;
                }
