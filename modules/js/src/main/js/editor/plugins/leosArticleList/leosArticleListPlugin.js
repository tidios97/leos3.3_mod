 ...
 * That's why this copy of the default list plugin was created to use the customized indentlist now called "leosArticleIndentlist" and avoid conficts.
 * Here are the main customizations:
 *     - Change the pluginName from "list" to leosArticleList
 *     - Removed "lang", "icons" and buttons from the plugin definition to avoid conflicts with the default one
 *     - Renamed the required plugin from "indentlist" to "leosArticleIndentlist".
 *     - Changed reference of arrayToList and listToArray from list plugin to leosAricleList plugin
 */

; // jshint ignore:line
define(function leosArticleListPluginModule(require) {
    "use strict";
    ...
    CKEDITOR.plugins.leosArticleList = {
        /**
        ...
        listToArray: function( listNode, database, baseArray, baseIndentLevel, grandparentNode ) {
            ...
            for ( var i = 0, count = listNode.getChildCount(); i < count; i++ ) {
                ...
                if ( listItem.type == CKEDITOR.NODE_ELEMENT && listItem.getName() in CKEDITOR.dtd.$list )
                    CKEDITOR.plugins.leosArticleList.listToArray( listItem, database, baseArray, baseIndentLevel + 1 );
                ...
                for ( var j = 0, itemChildCount = listItem.getChildCount(), child; j < itemChildCount; j++ ) {
                    ...
                    CKEDITOR.plugins.leosArticleList.listToArray( child, database, baseArray, baseIndentLevel + 1, itemObj.grandparent );
                    else
            ...
            while ( 1 ) {
                ...
                if ( item.indent == indentLevel ) {
                    if ( !rootNode || listArray[ currentIndex ].parent.getName() != rootNode.getName() ) {
                        var parentId = listArray[ currentIndex ].parent.getAttribute( 'id' );
                        rootNode = listArray[ currentIndex ].parent.clone( false, !(parentId && retval.findOne( '#' + parentId )) );
                    ...
                    var currDir = listArray[ currentIndex - 1 ].element.getDirection( 1 ),
                        listData = CKEDITOR.plugins.leosArticleList.arrayToList( listArray, null, currentIndex, paragraphMode, currDir != orgDir ? orgDir : null );
    ...
    function changeListType( editor, groupObj, database, listsCreated ) {
        ...
        var listArray = CKEDITOR.plugins.leosArticleList.listToArray( groupObj.root, database ),
            selectedListItems = [];
        ...
        var newList = CKEDITOR.plugins.leosArticleList.arrayToList( listArray, database, null, editor.config.enterMode );
        var child,
    ...
    function removeList( editor, groupObj, database ) {
        ...
        var newList = CKEDITOR.plugins.leosArticleList.arrayToList( listArray, database, null, editor.config.enterMode, groupObj.root.getAttribute( 'dir' ) );
