...
(function(root, factory) {
    ...
    function _initialize(target, selectorStyleMap) {
        log.debug("Sliderpin _initialize..." + target);
    ...
    function _destroy(target) {
        log.debug(" Sliderpin destroy..." + target.SliderPins);
    ...
    function _navigate(target, direction) {
        ...
        let scrolledPin  = _pins.find(function(pin, index, _pins) {
        ...
        return scrolledPin
            ? scrolledPin.getAttribute("data-index")
            : -1;
    }
    ...
    function checkElementInViewport(element,target, direction) {
        ...
        return direction === NAVIGATION.NEXT
            ? elementTop > (targetTop + 77)
            : elementTop < (targetTop + 75);
        //77 & 75 are offsets added based on the scrollToChange() method which adds an offset of 76.
