...
function shouldShowInTab(ann, tab) {
  ...
  return tabForAnnotation(ann) === tab && (!ann.status || ann.status.status == ANNOTATION_STATUS.NORMAL);
}
...
function counts(annotations) {
  var counts = {
    ...
    annotations: countIf(annotations, metadata.isAnnotationAndNotProcessed),
