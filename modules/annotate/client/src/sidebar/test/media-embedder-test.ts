...
type ElementWithSrc = Element & { src: any };
...
describe('media-embedder', function () {
  ...
  it('replaces YouTube watch links with iframes', function () {
    ...
    urls.forEach(function (url) {
      ...
      const elementWithSrc = element.children[0] as ElementWithSrc;
      assert.equal(elementWithSrc.tagName, 'IFRAME', url);
      assert.equal(
        elementWithSrc.src,
  ...
  it ('excludes non-whitelisted params in YouTube watch links', function () {
    ...
    urls.forEach(function (url) {
      ...
      assert.equal(element.childElementCount, 1);
      const elementWithSrc = element.children[0] as ElementWithSrc;
      assert.equal(elementWithSrc.tagName, 'IFRAME');
      // queryString's #stringify sorts keys, so resulting query string
      // will be reliably as follows, regardless of original ordering
      assert.equal(
        elementWithSrc.src,
  ...
  it('replaces YouTube share links with iframes', function () {
    ...
    urls.forEach(function (url) {
      ...
      assert.equal(elementWithSrc.tagName, 'IFRAME');
      assert.equal(
        elementWithSrc.src, 'https://www.youtube.com/embed/QCkm0lL-6lc');
    });
  });
  ...
  it('replaces Vimeo links with iframes', function () {
    ...
    urls.forEach(function (url) {
      ...
      assert.equal(elementWithSrc.tagName, 'IFRAME');
      assert.equal(
        elementWithSrc.src, 'https://player.vimeo.com/video/149000090');
    });
  });
  ...
  it('replaces Vimeo channel links with iframes', function () {
    ...
    urls.forEach(function (url) {
      ...
      assert.equal(elementWithSrc.tagName, 'IFRAME');
      assert.equal(
        elementWithSrc.src, 'https://player.vimeo.com/video/148845534');
    });
  });
  ...
  it('replaces audio links with html5 audio elements', function() {
    var urls = [
      ...
      'https://wisc.pb.unizin.org/frenchcscr/wp-content/uploads/sites/208/2018/03/6L�ry_Conclusion.mp3',
    ...
    urls.forEach(function (url) {
      ...
      assert.equal(element.childElementCount, 1);
      const elementWithSrc = element.children[0] as ElementWithSrc;
      assert.equal(elementWithSrc.tagName, 'AUDIO');
      assert.equal(elementWithSrc.src, encodedURL);
    });
  });
  ...
  it('replaces multiple links with multiple embeds', function () {
    ...
    const elementWithSrc01 = element.children[0] as ElementWithSrc;
    const elementWithSrc02 = element.children[1] as ElementWithSrc;
    assert.equal(elementWithSrc01.tagName, 'IFRAME');
    assert.equal(elementWithSrc01.src, 'https://www.youtube.com/embed/QCkm0lL-6lc');
    assert.equal(elementWithSrc02.tagName, 'IFRAME');
    assert.equal(elementWithSrc02.src, 'https://www.youtube.com/embed/abcdefg');
  });
});
