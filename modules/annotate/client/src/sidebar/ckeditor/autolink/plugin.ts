	...
	CKEDITOR.plugins.add( 'autolink', {
		...
		init: function( editor ) {
			...
			editor.on( 'paste', function( evt ) {
				...
				// The editor already encodes html entities on paste. However, during construction of the actual
				// link (getHtmlToInsert) this is done once again.
				// Therefore, we should revert the html encoding before constructing the link.
				var decodedLink = htmlEntitiesToString( data );

				if ( matchLink( decodedLink ) ) {
					evt.data.dataValue = getHtmlToInsert( decodedLink );
					evt.data.type = 'html';
				}
			} );
			...
			function htmlEntitiesToString( htmlEntities ) {
               	return String(htmlEntities).replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"');
            }
		}
	} );
