...
( function() {
	...
	CKEDITOR.plugins.textMatch = {
		match: match,
		getTextAndOffset: getTextAndOffset,
        getAdjacentTextNodes: getAdjacentTextNodes,
        getRangeInText: getRangeInText
	};
	 ...
	 */
	function match( range, callback ) {
		var textAndOffset = CKEDITOR.plugins.textMatch.getTextAndOffset( range ),
			fillingCharSequence = CKEDITOR.tools.repeat( '\u200b', 7 ), // TODO: There is no static property FILLING_CHAR_SEQUENCE on CKEDITOR.dom.selection?
	 ...
	 */
	function getTextAndOffset ( range ) {
		if ( !range.collapsed ) {
			return null;
		}
	 ...
	 */
	function getRangeInText ( range, start, end ) {
	 ...
	 */
	function getAdjacentTextNodes ( range ) {
		if ( !range.collapsed ) {
