...
var update = {
  ...
  UPDATE_ANCHOR_STATUS: function (state, action) {
    var annotations = state.annotations.map(function (annot) {
      ...
      if (state.anchoringStatus === 'timeout') {
        return Object.assign({}, annot, { $anchorTimeout: true });
      } else {
        return Object.assign({}, annot, {
          $orphan: state.anchoringStatus === 'orphan',
          anchoredRangeText: state.anchoredRangeText
        });
      }
    });
    return {annotations: annotations};
...
function updateAnnotationStatus(id, status) {
	return {
		type: actions.UPDATE_ANNOTATION_STATUS,
		id: id,
		status: status
	};
}
    ...
    if (anchoringIDs.length > 0) {
      setTimeout(() => {
        ...
        var anchorStatusUpdates = annsStillAnchoring.reduce((updates, ann) => {
          updates[ann.$tag] = { anchoredRangeText: updates[ann.$tag] !== undefined ? updates[ann.$tag].anchoredRangeText : undefined , anchoringStatus: 'timeout'} ;
          return updates;
        }, {});
...
function findAnnotationByTag(state, tag) {
  return findByTag(state.annotations, tag);
}

function getAnnotationCount(state) {
