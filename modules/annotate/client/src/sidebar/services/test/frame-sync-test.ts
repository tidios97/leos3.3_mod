...
describe('sidebar.frame-sync', function () {
  ...
  before(function () {
    angular.module('app', [])
      ...
      .value('rootThread', fakeRootThread);
  });
  ...
  context('when anchoring completes', function () {
    ...
    it('updates the anchoring status for the annotation', function () {
      fakeBridge.emit('sync', [{tag: 't1', msg: {$orphan: false, anchoredRangeText: 'anchor text'}}]);

      expireDebounceTimeout();

      assert.calledWith(fakeStore.updateAnchorStatus, { t1: {anchoringStatus: 'anchored', anchoredRangeText: 'anchor text' }});
    });

    it('coalesces multiple "sync" messages', () => {
      fakeBridge.emit('sync', [{tag: 't1', msg: {$orphan: false, anchoredRangeText: 'test text'}}]);
      fakeBridge.emit('sync', [{tag: 't2', msg: {$orphan: true}}]);

      expireDebounceTimeout();

      assert.calledWith(fakeStore.updateAnchorStatus, {
        t1: { anchoredRangeText: 'test text', anchoringStatus: "anchored" },
        t2: { anchoredRangeText: undefined, anchoringStatus: "orphan" }
      });
    });
