...
function translateResponseToError(response) {
  ...
  var err = new Error(message) as Error & { response: any };
...
function api($http, $q, apiRoutes, auth) {
  ...
  return {
    ...
    suggestion: {
        accept: apiCall('suggestion.accept'),
        reject: apiCall('suggestion.reject')
    },
    annotation: {
      ...
      deleteMultiple: apiCall('annotation.deleteMultiple'),
      treat: apiCall('annotation.treat'),
      treatMultiple: apiCall('annotation.treatMultiple'),
      reset: apiCall('annotation.reset'),
      resetMultiple: apiCall('annotation.resetMultiple')
    },
    group: {
      member: {
