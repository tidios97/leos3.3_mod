  ...
  function handleAnnotationNotification(message) {
    ...
    case 'past':
      annotations.forEach(function (ann) {
        ...
        const currentAuth = !!settings && !!settings.services[0] ? settings.services[0].authority : null;
        ...
        if (!currentAuth || !annAuth || currentAuth == annAuth) {
          ...
          groups.all().forEach(function (group) {
            if ((ann.group === group.id) || !store.isSidebar()) {
