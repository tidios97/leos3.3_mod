  ...
  describe('AnnotationController', function () {
    ...
    before(function () {
      angular.module('h', [])
        ...
        .component('ckeditorMarkdown', {
          bindings: require('../ckeditor-markdown').bindings,
        });
    });
    ...
    beforeEach(angular.mock.module(function ($provide) {
      ...
      fakeAnnotationMapper = {
        createAnnotation: sinon.stub().returns({
          permissions: {
        ...
        deleteAnnotation: sinon.stub(),
        flagAnnotation: sinon.stub(),
      };

      fakeStore = {
        updateFlagStatus: sinon.stub().returns(true),
      };
      ...
      fakeFeatures = {
        flagEnabled: sinon.spy(function(propName) {
          return false;
        })
      };
    ...
    describe('#forward', function() {
      it('annotation is considered forwarded', function() {
        var annotation = fixtures.defaultAnnotation();
        annotation.forwarded = true;
        var controller = createDirective(annotation).controller;
        assert.isTrue(controller.isForward());
        assert.isTrue(controller.isAnnotationForwarded());
      });
    });
    ...
    describe('#delete()', function () {
      beforeEach(function () {
        fakeAnnotationMapper.deleteAnnotation = sinon.stub();
      });
    ...
    describe('#flag()', function () {
      beforeEach(function () {
        fakeAnnotationMapper.flagAnnotation = sinon.stub();
      });
    ...
    it('renders quotes as plain text', function () {
      ...
      assert.equal(el[0].querySelector('blockquote').textContent.trim(), '<<-&->>');
    });
    ...
    unroll('renders hidden annotations with a custom text class (#context)', function (testCase) {
      ...
      assert.match(el.find('ckeditor-markdown').controller('ckeditorMarkdown'), sinon.match({
