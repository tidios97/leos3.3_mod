...
var proxyquire = require('proxyquire');
  ...
  beforeEach(angular.mock.module(function ($provide) {
    ...
    fakeStreamFilter = {
      ...
      addClause: sinon.stub().returnsThis(),
      getFilter: sinon.stub().returns({}),
    };

    fakeGroups = {
      defaultGroupId: sinon.stub(),
  ...
  describe('#loadAnnotations', function () {
    it('unloads any existing annotations', function () {
      ...
      setFrames(frames);
      $scope.$digest();
      fakeAnnotationMapper.unloadAnnotations = sinon.spy();
  ...
  function connectFrameAndPerformInitialFetch() {
    ...
    $scope.$digest();
    fakeAnnotationMapper.loadAnnotations.resetHistory();
  }
