...
describe('annotator.adder', function () {
  ...
  describe('when Shadow DOM is supported', function () {
    it('creates the adder DOM in a shadow root for Shadow DOM v0 API', function () {
      var shadowEl;
      adderEl.attachShadow = undefined; // Overwrite native browser code.
      adderEl.createShadowRoot = sinon.spy(function () {
      ...
      assert.called(adderEl.createShadowRoot);
      assert.equal(shadowEl.childNodes[0].tagName.toLowerCase(), 'hypothesis-adder-toolbar');
    });

    it('creates the adder DOM in a shadow root for Shadow DOM v1 API', function () {
      var shadowEl;
      adderEl.attachShadow = sinon.spy(function () {
        shadowEl = document.createElement('shadow-root');
        adderEl.appendChild(shadowEl);
        return shadowEl;
      });
  ...
  describe('button handling', function () {
    it('calls onHighlight callback when Highlight button is clicked', function () {
      adderCtrl.element.querySelector('.js-highlight-btn').dispatchEvent(new Event('click'));
