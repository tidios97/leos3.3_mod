...
var fixture = require('./anchoring-fixture.html');
let testUtils = require('./common-test-utils');
...
describe('HTML anchoring', function () {
  ...
  unroll('describes and anchors "#description"', function (testCase) {
    ...
    // text was selected.
    var range = toRange(dummyHtmlElements.container, testCase.range);
    assert.equal(range.toString(), testCase.quote);

    // Capture a set of selectors describing the range and perform basic sanity
    // checks on them.
    var selectors = leos.describe(dummyHtmlElements.container, range);
    ...
    var anchored = selectors.map(function (sel) {
      if (sel.type != 'FragmentSelector' && sel.type != 'LeosSelector') {
        return leos.anchor(dummyHtmlElements.container, [sel]).then(function (anchoredRange) {
