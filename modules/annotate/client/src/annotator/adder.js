...
var leos_template = require('./leos-adder.html');
...
class Adder {
  ...
  extend(container, hostBridge, options) {
    ...
    let suggestBtn = this.element.querySelector(SUGGESTION_BTN_SELECTOR);
    let commentBtn = this.element.querySelector(COMMENT_BTN_SELECTOR);
    let highlightBtn = this.element.querySelector(HIGHLIGHT_BTN_SELECTOR);
    if (highlightBtn) {
  ...
  disableCommentButton() {
    let button = this.element.querySelector(COMMENT_BTN_SELECTOR);
    if (button) {
      button.onclick = null;
  ...
  enableSuggestionButton() {
    let button = this.element.querySelector(SUGGESTION_BTN_SELECTOR);
    if (button) {
  ...
  disableHighlightButton() {
    let button = this.element.querySelector(HIGHLIGHT_BTN_SELECTOR);
    if (button) {
      button.onclick = null;
      button.classList.add("annotator-disabled");
