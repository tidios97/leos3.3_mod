...
public class AnnotateWebSocketHandler extends TextWebSocketHandler {
    ...
    private void handlePing(final Map<String, Object> jsonObject, final WebSocketSession session) throws IOException {
    ...
    public Map<String, String> decodeQueryString(final String query) {
        try {
            ...
            for (final String param : query.split("&")) {
                ...
                final String key = URLDecoder.decode(keyValue[0], StandardCharsets.UTF_8.name());
                final String value = keyValue.length > 1 ? URLDecoder.decode(keyValue[1], StandardCharsets.UTF_8.name()) : "";
                if (!key.isEmpty()) {
