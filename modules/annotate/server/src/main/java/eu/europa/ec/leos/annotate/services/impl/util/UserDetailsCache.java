...
public class UserDetailsCache {
     ...
     * @param context the context serving as cache key
     * @return found UserDetails object, or null
     */
    public UserDetails getCachedUserDetails(final String login, final String context) {
        ...
        return udcache.get(login + (StringUtils.hasLength(context) ? "#" + context : ""));
    }
     ...
     * @param context the user's context, serving as cache key
     * @param details the details of the user
     */
    public void cache(final String login, final String context, final UserDetails details) {
        ...
        if (details == null) {
            LOG.warn("Cannot cache empty user details for key {} - {}", login, context);
            return;
        }

        udcache.put(login + (StringUtils.hasLength(context) ? "#" + context : ""), details);
    }
