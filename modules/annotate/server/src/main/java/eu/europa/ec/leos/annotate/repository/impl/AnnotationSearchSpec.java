...
public class AnnotationSearchSpec implements Specification<Annotation> {
     ...
     *        the {@link SearchModelMode}; indicates if specific items have to be filtered out (used in ISC context)
     * @param shared
     *        flag indicating if public (true), private (false) or all (null) annotations are wanted
     */
    public AnnotationSearchSpec(final long executingUserId,
                                final Long optionalUserIdToFilter,
                                final List<MetadataIdsAndStatuses> metadataAndStatus,
                                final Group group, final SearchModelMode searchModeToUse,
                                final Boolean shared) {
        this(executingUserId, null, optionalUserIdToFilter, metadataAndStatus, group, searchModeToUse, shared);
    }
    ...
    public Predicate toPredicate(final Root<Annotation> root, final CriteriaQuery<?> query, final CriteriaBuilder critBuilder) {
        ...
        final List<Predicate> metadataStatusPreds = new ArrayList<>();
        for (final MetadataIdsAndStatuses mias : this.metadataIdsAndStatuses) {

            Predicate metaPred;
            LOG.trace("filter metadataIds={}", mias.getMetadataIds());
            ...
            LOG.trace("filter status={}", mias.getStatuses());
        ...
        // Support users can access all annotations
        ...
        // ANOT-125
        // finally, we may filter on public or private annotations, if requested
        if(this.shared != null) {
            predicates.add(critBuilder.equal(root.<Boolean>get("shared"), this.shared));
        }
