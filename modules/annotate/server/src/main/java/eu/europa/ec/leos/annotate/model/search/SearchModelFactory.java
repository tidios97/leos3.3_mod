...
public class SearchModelFactory {
    ...
    public SearchModel getSearchModel(final ResolvedSearchOptions rso) {
        ...
        if (Authorities.isLeos(authority) || Authorities.isSupport(authority)) {
            ...
            if (defaultGroupName.equals(rso.getGroup().getName())) {
                ...
                if (useMetadataSearch) {
                    ...
                    // case EdiT.1
                    // -------------------------------------
                    resultingModel = getSearchModelEdiT1(rso, authority);
                }
            } else {
    ...
    private SearchModel getSearchModelEdiTByMetadata(final ResolvedSearchOptions rso) {
        ...
        final List<Long> groupIds = userGroupService.getGroupIdsOfUser(rso.getExecutingUser()); // won't be empty as at least the default group is present
        ...
        final List<MetadataIdsAndStatuses> resolvedCandidates = new ArrayList<>();
    ...
    private SearchModel getSearchModelEdiT1(
            final ResolvedSearchOptions rso,
            final String authority) {

        final List<Long> groupIds = userGroupService.getGroupIdsOfUser(
                rso.getExecutingUser(), authority); // won't be empty as at least the default group is present
    ...
    private SearchModel getSearchModelIsc1And2(final ResolvedSearchOptions rso) {
        ...
        List<Metadata> metaCandidatesInPrep = new ArrayList<>();
