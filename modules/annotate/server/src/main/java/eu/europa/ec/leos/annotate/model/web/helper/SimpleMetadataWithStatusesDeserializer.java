...
public class SimpleMetadataWithStatusesDeserializer extends StdDeserializer<SimpleMetadataWithStatuses> {
     ...
     */
    @Override
    public SimpleMetadataWithStatuses deserialize(final JsonParser jparser, final DeserializationContext ctxt) {
        ...
        try {
            ...
            while (iterField.hasNext()) {
                ...
                if (currentField.getKey().equals("status")) {
                    statuses = JsonStringDeserializer.convertJsonToAnnotationStatusList(currentField.getValue().toString());
                } else {
