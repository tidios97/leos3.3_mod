...
public class OffsetBasedPageRequest implements Pageable, Serializable {
    ...
    private final int limit; // the number of items to return
    private final long offset; // number of items to skip
    ...
    public OffsetBasedPageRequest(final long offset, final int limit, final Sort sort) {
        ...
        Assert.isTrue(offset >= 0, "Offset index must not be less than zero!");
        Assert.isTrue(limit >= 1, "Limit must not be less than one!");
    ...
    @Override public Pageable withPage(int pageNumber) {
        return new OffsetBasedPageRequest(getOffset() + (pageNumber - 1) * getPageSize(), getPageSize(), getSort());
    }
