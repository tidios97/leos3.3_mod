...
public class StatusUpdateTest {
    ...
    public void testResponseStatusUpdateOk() throws Exception {
        ...
        jsAnnot.getDocument().getMetadata().put(Metadata.PROP_ISC_REF, ISC_REF_VAL);
        jsAnnot.getDocument().getMetadata().put(Metadata.PROP_RESPONSE_VERSION, RESP_VERS_VAL);
        jsAnnot.getDocument().getMetadata().put(Metadata.PROP_RESPONSE_ID, RESP_ID_VAL);
        jsAnnot.getDocument().getMetadata().put(Metadata.PROP_RESPONSE_STATUS, ResponseStatus.IN_PREPARATION.toString());
        ...
        Assert.assertEquals(ResponseStatus.IN_PREPARATION, readAnnot.getMetadata().getResponseStatus());
        ...
        readMetaSent.setResponseStatus(ResponseStatus.SENT);
        ...
        metaMap.put(Metadata.PROP_ISC_REF, ISC_REF_VAL);
        metaMap.put(Metadata.PROP_RESPONSE_VERSION, RESP_VERS_VAL);
        metaMap.put(Metadata.PROP_RESPONSE_ID, RESP_ID_VAL);
        metaMap.put(Metadata.PROP_RESPONSE_STATUS, ResponseStatus.IN_PREPARATION.toString());
        ...
        // - the metadata status change was tracked (time, user, group)
        // - it is no longer linked to another annotation
        readAnnot = annotService.findAnnotationById(annotId);
        Assert.assertNotNull(readAnnot);
        Assert.assertEquals(ResponseStatus.SENT, readAnnot.getMetadata().getResponseStatus());
        Assert.assertTrue(TestHelper.withinLastSeconds(readAnnot.getMetadata().getResponseStatusUpdated(), 5));
        Assert.assertTrue(TestHelper.withinLastSeconds(readAnnot.getUpdated(), 5));
        Assert.assertEquals(user.getId(), readAnnot.getMetadata().getResponseStatusUpdatedByUser());
        Assert.assertEquals(readAnnot.getMetadata().getGroupId(), readAnnot.getMetadata().getResponseStatusUpdatedByGroup().longValue());
    ...
    /**
     * successfully increase the response version, expected HTTP 200 and success message
     */
    @SuppressFBWarnings(value = SpotBugsAnnotations.FieldNotInitialized, justification = SpotBugsAnnotations.FieldNotInitializedReason)
    @Test
    public void testResponseStatusVersionMigrationOk() throws Exception {

        final String ISC_REF_VAL = "ISC/2016/642";
        final String RESP_ID_VAL = "id3";
        final String RESP_VERS_VAL = "4";
        final Token token = new Token(user, Authorities.ISC, ACCESS_TOKEN, LocalDateTime.now().plusMinutes(5), REFRESH_TOKEN,
                LocalDateTime.now().plusMinutes(5));
        tokenRepos.save(token);
        final UserInformation userInfo = new UserInformation(token);
        ...
        // manipulate its "updated" data to be in the past
        readAnnot.setUpdated(LocalDateTime.now().minusDays(5));
        annotRepos.save(readAnnot);

        // launch a request to update the response status by increasing the version
        final SimpleMetadata metaMap = new SimpleMetadata();
        metaMap.put(Metadata.PROP_ISC_REF, ISC_REF_VAL);
        metaMap.put(Metadata.PROP_RESPONSE_VERSION, RESP_VERS_VAL);
        metaMap.put(Metadata.PROP_RESPONSE_ID, RESP_ID_VAL);
        ...
        final MockHttpServletRequestBuilder builder = MockMvcRequestBuilders
                .post("/api/changeStatus?group=" + jsAnnot.getGroup() + "&uri=" + jsAnnot.getDocument().getLink().get(0).getHref().toString() +
                        "&responseStatus=IN_PREPARATION&migrateVersion=true")
                .header(TestHelper.AUTH_HEADER, TestHelper.AUTH_BEARER + ACCESS_TOKEN)
                .contentType(MediaType.APPLICATION_JSON)
                .content(serializedMetadataToMatch);
        ...
        final StatusUpdateSuccessResponse jsResponse = SerialisationHelper.deserializeJsonStatusUpdateSuccessResponse(responseString);
        Assert.assertNotNull(jsResponse);
        Assert.assertTrue(jsResponse.getUpdated());

        // verify
        // - the annotation's metadata status was not changed (remains IN_PREPARATION)
        // - the metadata response version was increased to 5
        readAnnot = annotService.findAnnotationById(annotId);
        Assert.assertNotNull(readAnnot);
        Assert.assertEquals(ResponseStatus.IN_PREPARATION, readAnnot.getMetadata().getResponseStatus());
        Assert.assertEquals(5, MetadataHandler.getResponseVersion(readAnnot.getMetadata()));
        Assert.assertTrue(TestHelper.withinLastSeconds(readAnnot.getUpdated(), 5));
    }
}
