...
public class SearchAndCountAnnotationsWithMetadataSetsAndStatusesTest {
    ...
    private Group theGroup, deletingGroup;
    ...
    public void setupTests() throws URISyntaxException {
        ...
        deletingGroup = new Group("deletingGroup", true);
        groupRepos.save(deletingGroup);
        ...
        userDetailsCache.cache(LOGIN1, null, userDetails);
        ...
        userDetailsCache.cache(LOGINACC, null, new UserDetails(LOGINACC, userAcc.getId(), "acceptor", "the", entitiesAcc, "ac@ec.eu", null));
        userDetailsCache.cache(LOGINDEL, null, new UserDetails(LOGINDEL, userDel.getId(), "deletor", "the", entitiesDel, "del@ec.eu", null));
    }
    ...
    private void createTestData() throws CannotCreateAnnotationException, URISyntaxException {
        ...
        // create replies
        ann1R1N = createReply(Arrays.asList(ann1.getId()), document, theGroup, AnnotationStatus.NORMAL, null, null, null);
        ann1R2D = createReply(Arrays.asList(ann1.getId()), document, theGroup, AnnotationStatus.DELETED, userDel, null, LocalDateTime.of(2019, 8, 24, 13, 24));
        ann2R1N = createReply(Arrays.asList(ann2.getId()), document, theGroup, AnnotationStatus.NORMAL, null, null, null);
        ann2R2N = createReply(Arrays.asList(ann2.getId()), document, theGroup, AnnotationStatus.NORMAL, null, null, null);
        ann3R1N = createReply(Arrays.asList(ann3.getId()), document, theGroup, AnnotationStatus.NORMAL, null, null, null); // cannot occur in real life
        ann4R1D = createReply(Arrays.asList(ann4.getId()), document, theGroup, AnnotationStatus.DELETED, userDel, null, LocalDateTime.of(2019, 8, 24, 13, 24));
        ann4R2D = createReply(Arrays.asList(ann4.getId()), document, theGroup, AnnotationStatus.DELETED, userDel, deletingGroup,
                LocalDateTime.of(2019, 8, 24, 13, 24));
        ann5R1A = createReply(Arrays.asList(ann5.getId()), document, theGroup, AnnotationStatus.ACCEPTED, userAcc, null, LocalDateTime.of(2018, 6, 25, 12, 25));
        ann5R2D = createReply(Arrays.asList(ann5.getId()), document, theGroup, AnnotationStatus.DELETED, userDel, deletingGroup,
                LocalDateTime.of(2019, 8, 24, 13, 24));
        ...
        annotRepos.saveAll(Arrays.asList(ann3, ann4, ann5, ann6));
        ...
        metadataRepos.findAll().forEach(meta -> {
            meta.setResponseStatus(ResponseStatus.SENT);
            metadataRepos.save(meta);
        });
    }

    private void setAnnotationStatus(final Annotation ann, final AnnotationStatus newStatus, final User user,
            final LocalDateTime dateTime) {

        ann.setStatus(newStatus);
        ann.setStatusUpdated(dateTime);
        ann.setStatusUpdatedByUser(user.getId());
        // note: group of status change not relevant here
    }
    ...
    private Annotation createAnnotation(final Metadata meta, final String annotId) throws CannotCreateAnnotationException {
        ...
        final String initialAnnotId = annot.getId();
        ...
        annotRepos.deleteById(initialAnnotId);
    ...
    private Annotation createReply(final List<String> parentAnnotIds, final Document document,
            ...
            final User statusModifyingUser, final Group statusModifyingGroup,
            final LocalDateTime statusModificationDate)
            throws CannotCreateAnnotationException, URISyntaxException {

        final JsonAnnotation jsReply = TestData.getTestReplyToAnnotation(USER_ACCOUNT,
        ...
        reply.setStatusUpdatedByUser(statusModifyingUser == null ? null : statusModifyingUser.getId());
        reply.setStatusUpdatedByGroup(statusModifyingGroup == null ? null : statusModifyingGroup.getId());
        reply.setStatusUpdated(statusModificationDate);
        annotRepos.save(reply);
    ...
    private Metadata createMetadata(final Document document, final Group group,
        ...
        MetadataHandler.setKeyValuePropertyFromSimpleMetadata(meta, metaMap);
        metadataRepos.save(meta);
    ...
    public void testSearch_Edit_IscRef1Resp1NormalAccepted_Or_IscRef2DeletedAccepted() throws Exception {
        ...
        for (final Metadata meta : allMeta) {
            if (MetadataHandler.getKeyValuePropertyAsSimpleMetadata(meta).size() == 0) {
                meta.setSystemId(Authorities.EdiT);
                MetadataHandler.setKeyValuePropertyFromSimpleMetadata(meta, new SimpleMetadata("key", "value")); // create some metadata
            } else {
                meta.setResponseStatus(ResponseStatus.SENT);
            }
        }
        metadataRepos.saveAll(allMeta);
    ...
    private void runSearchInternal(final String serializedMetaRequest,
        ...
        MatcherAssert.assertThat(expectedAnnotationIds,
        ...
        MatcherAssert.assertThat(expectedReplyIds,
    ...
    private void checkStatus(final Annotation annRef, final JsonAnnotation foundAnnot, final boolean checkReplyDetails) {
        ...
        if (checkReplyDetails) {
            if (annRef.getStatusUpdatedByUser() == null) {
                ...
                if (annRef.getStatusUpdatedByUser().longValue() == userAcc.getId().longValue()) {
                ...
                } else if (annRef.getStatusUpdatedByUser().longValue() == userDel.getId().longValue()) {
                    Assert.assertEquals(USER_ACCOUNT_DEL, foundAnnot.getStatus().getUser_info());

                    if (annRef.getStatusUpdatedByGroup() != null) {
                        Assert.assertEquals(groupRepos.findById(annRef.getStatusUpdatedByGroup()).get().getName(),
                                foundAnnot.getStatus().getUpdated_by());
                    }
                }
            }
        }
