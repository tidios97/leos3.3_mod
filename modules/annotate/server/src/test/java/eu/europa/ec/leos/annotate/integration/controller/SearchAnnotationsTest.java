...
public class SearchAnnotationsTest {
    ...
    public void testSearchAnnotationsNoSeparateReplies() throws Exception {
        ...
        final JsonAnnotation jsAnnotFirst = TestData.getTestAnnotationObject(USER1);
        final Annotation annotFirst = annotService.createAnnotation(jsAnnotFirst, userInfo1);
        final String idFirst = annotFirst.getId();

        final JsonAnnotation jsAnnotReply = TestData.getTestReplyToAnnotation(USER2, jsAnnotFirst.getUri(), Arrays.asList(idFirst));
        final Annotation annotReply = annotService.createAnnotation(jsAnnotReply, userInfo2);
        final String idReply = annotReply.getId();

        // save a second annotation
        final JsonAnnotation jsAnnotSecond = TestData.getTestAnnotationObject(USER2);
        final Annotation annotSecond = annotService.createAnnotation(jsAnnotSecond, userInfo2);
        final String idSecond = annotSecond.getId();

        // send search request
        final MockHttpServletRequestBuilder builder = MockMvcRequestBuilders
                .get("/api/search?_separate_replies=false&sort=created&order=asc&uri=" + annotFirst.getDocument().getUri().toString() + WORLD)
        ...
        MatcherAssert.assertThat(Arrays.asList(idFirst, idReply, idSecond),
    ...
    public void testSearchAnnotationsWithSeparateReplies() throws Exception {
        ...
        final JsonAnnotation jsAnnotReply = TestData.getTestReplyToAnnotation(USER2, new URI(annotFirst.getDocument().getUri()), Arrays.asList(idFirst));
        final Annotation annotReply = annotService.createAnnotation(jsAnnotReply, userInfo2);
        final String idReply = annotReply.getId();
        ...
        // send search request
        final MockHttpServletRequestBuilder builder = MockMvcRequestBuilders
                .get(SEARCH_URL + annotFirst.getDocument().getUri().toString() + WORLD)
                .header(TestHelper.AUTH_HEADER, TestHelper.AUTH_BEARER + ACCESS_TOKEN);
        final ResultActions result = this.mockMvc.perform(builder);
        ...
        MatcherAssert.assertThat(Arrays.asList(idFirst, idSecond),
                containsInAnyOrder(jsResponse.getRows().stream().map(JsonAnnotation::getId).toArray()));
    ...
    // NOTE: TEMPORARILY COMMENTED OUT (ANOT-134)
//    /**
//     * successfully search for annotations, expected HTTP 200 and the annotation data
...
//    }
    ...
    public void testSearchAnnotationsWithSeveralMetadataSetsWithVersion() throws Exception {
        ...
        // preparation: save an annotation and a reply for it
        final JsonAnnotation jsAnnotFirst = TestData.getTestAnnotationObject(USER_ACCOUNT);
        jsAnnotFirst.getDocument().getMetadata().put(VERSION, "4");
        final Annotation annotFirst = annotService.createAnnotation(jsAnnotFirst, userInfo1);
        final String idFirst = annotFirst.getId();

        final JsonAnnotation jsAnnotReply = TestData.getTestReplyToAnnotation(USER_ACCOUNT, new URI(annotFirst.getDocument().getUri()), Arrays.asList(idFirst));
        final Annotation annotReply = annotService.createAnnotation(jsAnnotReply, userInfo1);
        final String idReply = annotReply.getId();

        // save a second annotation having different metadata
        final JsonAnnotation jsAnnotSecond = TestData.getTestAnnotationObject(USER_ACCOUNT);
        jsAnnotSecond.getDocument().getMetadata().put(VERSION, "2");
        jsAnnotSecond.getDocument().getMetadata().put(REFERENCE, "theref");
        final Annotation annotSecond = annotService.createAnnotation(jsAnnotSecond, userInfo1);
        final String idSecond = annotSecond.getId();
    ...
    public void testSearchAnnotationsWithSeveralStatuses() throws Exception {
        ...
        final JsonAnnotation jsAnnotFirstReply = TestData.getTestReplyToAnnotation(USER_ACCOUNT, new URI(annotFirst.getDocument().getUri()),
                Arrays.asList(idFirst));
        final Annotation annotFirstReply = annotService.createAnnotation(jsAnnotFirstReply, userInfo1);
        final String idFirstReply = annotFirstReply.getId();
        ...
        final JsonAnnotation jsAnnotSecondReply = TestData.getTestReplyToAnnotation(USER_ACCOUNT, new URI(annotSecond.getDocument().getUri()),
                Arrays.asList(idSecond));
        final Annotation annotSecondReply = annotService.createAnnotation(jsAnnotSecondReply, userInfo1);
        final String idSecondReply = annotSecondReply.getId();
        annotService.deleteAnnotationById(idSecondReply, userInfo1);

        // send search request
        final MockHttpServletRequestBuilder builder = MockMvcRequestBuilders
                .get(SEARCH_URL + annotFirst.getDocument().getUri().toString() +
                        WORLD 
        ...
        // verify reply IDs
        MatcherAssert.assertThat(Arrays.asList(idFirstReply, idSecondReply),
