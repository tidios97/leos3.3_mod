...
public class UserDetailsCacheTest {
    ...
    public void testUserDetailsCache() {
        ...
        Assert.assertNull(userCache.getCachedUserDetails(LOGIN, null));

        final UserDetails details = new UserDetails(LOGIN, (long) 47, "Santa", "Clause", null, "santa@clause.europa.eu", null);
        userCache.cache(LOGIN, null, details);
        Assert.assertEquals(1, userCache.size());

        final UserDetails cachedItem = userCache.getCachedUserDetails(LOGIN, null);
    ...
    public void testCacheWithoutName() {
        ...
        Assert.assertNull(userCache.getCachedUserDetails("", null));

        // verify that no exception is thrown when trying to cache invalid value
        userCache.cache("", null, null);
    }
    ...
    public void testUserDetailsCacheCleanup() {
        ...
        final UserDetails details = new UserDetails(LOGIN, (long) 47, "Santa", "Clause", null, "santa@clause.europa.eu", null);
        userCache.cache(LOGIN, null, details);
        Assert.assertNotNull(userCache.getCachedUserDetails(LOGIN, null));
