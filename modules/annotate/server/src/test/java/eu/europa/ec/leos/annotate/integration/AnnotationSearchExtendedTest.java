...
public class AnnotationSearchExtendedTest {
    ...
    @Autowired
    private UserDetailsCache userCache; // used for speeding up the tests

    // -------------------------------------
    // Cleanup of database content and prepare test data
    // -------------------------------------
    private static final String DOC_URL = "uri://LEOS/doc1";
    private static final String FIRST_USER_LOGIN = "userLogin";
    private static final String SECOND_USER_LOGIN = "secondUser";
    private static final String THIRD_USER_LOGIN = "thirdUser";
    ...
    public void cleanDatabaseBeforeTests() throws URISyntaxException {
        ...
        userRepos.saveAll(Arrays.asList(firstUser, secondUser, thirdUser));
        ...
        final UserEntity entity = new UserEntity("9", "Entity name", "org name");
        userCache.cache(FIRST_USER_LOGIN, null, new UserDetails(FIRST_USER_LOGIN, (long) 1, "f", "irst", Arrays.asList(entity), "", null));
        userCache.cache(SECOND_USER_LOGIN, null, new UserDetails(SECOND_USER_LOGIN, (long) 2, "s", "econd", Arrays.asList(entity), "", null));
        userCache.cache(THIRD_USER_LOGIN, null, new UserDetails(THIRD_USER_LOGIN, (long) 3, "t", "hird", Arrays.asList(entity), "", null));

        // create user info here with reference to the firstUser object! (has cost hours of debugging and searching...)
        userInfo = new UserInformation(new Token(firstUser, AUTHORITY, "access", LocalDateTime.now().plusHours(1), "refr",
        ...
        AnnotationReferencesHandler.setReferences(nineteenthAnnot, Arrays.asList(ID_USER1_18_PUB, ID_USER1_19_PUB_REP_DEL));
        ...
        AnnotationReferencesHandler.setReferences(twentythirdAnnot, Arrays.asList(ID_USER1_20_PUB, ID_USER1_21_PUB_REPLY));
    ...
    public void testFirstBunchOfTenItems() {
        ...
        final AnnotationSearchResult annotations = annotSearchService.searchAnnotations(options, userInfo);
        Assert.assertEquals(10, annotations.size());
        Assert.assertEquals(29, annotations.getTotalItems());

        final List<Annotation> replies = annotSearchService.searchRepliesForAnnotations(annotations, options, userInfo);
    ...
    public void testSecondBunchOfTenItems() {
        ...
        final AnnotationSearchResult annots = annotSearchService.searchAnnotations(options, userInfo);
        Assert.assertEquals(10, annots.size());
        Assert.assertEquals(29, annots.getTotalItems());

        final List<Annotation> replies = annotSearchService.searchRepliesForAnnotations(annots, options, userInfo);
    ...
    /**
     * test: search for public annotations only
     */
    @Test
    public void testPublicOnly() {
        ...
        // 20 items found!
        final AnnotationSearchResult annots = annotSearchService.searchAnnotations(options, userInfo);
        Assert.assertEquals(20, annots.size());
        Assert.assertEquals(20, annots.getTotalItems());

        final List<Annotation> replies = annotSearchService.searchRepliesForAnnotations(annots, options, userInfo);
        Assert.assertEquals(5, replies.size());
        ...
        assertItemAndReplies(result.getRows().get(2), ID_USER1_3_PUB, null);
        assertItemAndReplies(result.getRows().get(3), ID_USER1_4_PUB, null);
        assertItemAndReplies(result.getRows().get(4), ID_USER1_10_PUB, null);
        assertItemAndReplies(result.getRows().get(5), ID_USER1_11_PUB, null);
        assertItemAndReplies(result.getRows().get(6), ID_USER1_12_PUB, null);
        assertItemAndReplies(result.getRows().get(7), ID_USER1_13_PUB, null);
        assertItemAndReplies(result.getRows().get(8), ID_USER1_14_PUB, null);
        assertItemAndReplies(result.getRows().get(9), ID_USER1_15_PUB, null);
        assertItemAndReplies(result.getRows().get(10), ID_USER1_16_PUB, null);
        assertItemAndReplies(result.getRows().get(11), ID_USER1_17_PUB, null);
        assertItemAndReplies(result.getRows().get(12), ID_USER1_18_PUB, null);
        assertItemAndReplies(result.getRows().get(13), ID_USER1_20_PUB, null);
        assertItemAndReplies(result.getRows().get(14), ID_USER2_26_PUB, null);
        assertItemAndReplies(result.getRows().get(15), ID_USER1_30_PUB_PNT, null);
        assertItemAndReplies(result.getRows().get(16), ID_USER1_31_PUB, null);
        assertItemAndReplies(result.getRows().get(17), ID_USER1_32_PUB, null);
        assertItemAndReplies(result.getRows().get(18), ID_USER3_34_PUB, null);
        assertItemAndReplies(result.getRows().get(19), ID_USER3_36_PUB, null);
    }

    /**
     * test: search for private annotations only
     */
    @Test
    public void testPrivateOnly() {
        ...
        final AnnotationSearchOptions options = new AnnotationSearchOptions(
                ...
                0, 0,                 // limit, offset
                ASC, CREATED);        // order, sort column
        options.setShared(false);

        // 10 items found!
        final AnnotationSearchResult annots = annotSearchService.searchAnnotations(options, userInfo);
        Assert.assertEquals(9, annots.size());
        Assert.assertEquals(9, annots.getTotalItems());
        ...
        // check that the correct items (in correct order) were returned and check their respective number of replies
        assertItemAndReplies(result.getRows().get(0), ID_USER1_5_PRIV, null);
        assertItemAndReplies(result.getRows().get(1), ID_USER1_6_PRIV, null);
        assertItemAndReplies(result.getRows().get(2), ID_USER1_7_PRIV, null);
        assertItemAndReplies(result.getRows().get(3), ID_USER1_8_PRIV, null);
        assertItemAndReplies(result.getRows().get(4), ID_USER1_9_PRIV, null);
        assertItemAndReplies(result.getRows().get(5), ID_USER1_24_PRIV, null);
        assertItemAndReplies(result.getRows().get(6), ID_USER1_25_PRIV, null);
        assertItemAndReplies(result.getRows().get(7), ID_USER1_27_PRIV_PNT, null);
        assertItemAndReplies(result.getRows().get(8), ID_USER1_33_PRIV, null);
    }
     ...
     */
    @Test
    public void testSecondBunchOfPublicOnly() {
        ...
        final AnnotationSearchResult annots = annotSearchService.searchAnnotations(options, userInfo);
        Assert.assertEquals(10, annots.size());
        Assert.assertEquals(20, annots.getTotalItems()); // there are 10 more
        ...
        // check that the correct items (in correct order) were returned and check their respective number of replies
        assertItemAndReplies(result.getRows().get(0), ID_USER1_16_PUB, null);
        assertItemAndReplies(result.getRows().get(1), ID_USER1_17_PUB, null);
        assertItemAndReplies(result.getRows().get(2), ID_USER1_18_PUB, null);
        assertItemAndReplies(result.getRows().get(3), ID_USER1_20_PUB, null);
        assertItemAndReplies(result.getRows().get(4), ID_USER2_26_PUB, null);
        assertItemAndReplies(result.getRows().get(5), ID_USER1_30_PUB_PNT, null);
        assertItemAndReplies(result.getRows().get(6), ID_USER1_31_PUB, null);
        assertItemAndReplies(result.getRows().get(7), ID_USER1_32_PUB, null);
        assertItemAndReplies(result.getRows().get(8), ID_USER3_34_PUB, null);
        assertItemAndReplies(result.getRows().get(9), ID_USER3_36_PUB, null);
    }
