...
public class SuggestionAcceptTest {
    ...
    @Autowired
    private AnnotationSuggestionService annotSuggestionService;
    ...
    public void testAcceptSimpleSuggestion() throws Exception {
        ...
        userDetailsCache.cache(theUser.getLogin(), null, userDetails);
        ...
        annotSuggestionService.acceptSuggestionById(annot.getId(), userInfo);
        ...
        Assert.assertEquals(1, documentRepos.count());
        Assert.assertEquals(1, tagRepos.count());
        TestHelper.assertHasStatus(annotRepos, annot.getId(), AnnotationStatus.ACCEPTED, theUser.getId(), null);
    ...
    public void testCannotAcceptSentSuggestionInIsc() throws Exception {
        ...
        final Metadata savedMeta = annotService.findAnnotationById(annot.getId()).getMetadata();
    ...
    public void testAcceptSimpleSuggestionWithReply() throws Exception {
        ...
        final JsonAnnotation jsReply = TestData.getTestReplyToAnnotation(HYPO_USER_ACCOUNT, new URI(annot.getDocument().getUri()),
        ...
        TestHelper.assertHasStatus(annotRepos, annot.getId(), AnnotationStatus.ACCEPTED, theUser.getId(), null);
        TestHelper.assertHasStatus(annotRepos, reply.getId(), AnnotationStatus.ACCEPTED, theUser.getId(), null);
    }
    ...
    public void testAcceptSimpleSuggestionWithDeletedReply() throws Exception {
        ...
        final Annotation savedReply = annotRepos.findById(reply.getId()).get();
        ...
        Assert.assertEquals(2, annotRepos.count());
        TestHelper.assertHasStatus(annotRepos, annot.getId(), AnnotationStatus.ACCEPTED, theUser.getId(), null);
        // null values since we manually manipulated status; AnnotationService was not involved
        TestHelper.assertHasStatus(annotRepos, reply.getId(), AnnotationStatus.DELETED, null, null);
    }
    ...
    public void testUndefinedSuggestionId() throws Exception {
        ...
        final UserInformation userInfo = new UserInformation(theUser, Authorities.ISC);
        annotSuggestionService.acceptSuggestionById(null, userInfo);
    ...
    public void testSuggestionNotFound() throws Exception {
        ...
        final UserInformation userInfo = new UserInformation(theUser, Authorities.ISC);
        annotSuggestionService.acceptSuggestionById("suggestionId", userInfo);
    ...
    public void testAcceptSuggestionWithoutUserLogin() throws Exception {
        ...
        // retrieve our test annotation
        final JsonAnnotation jsAnnot = TestData.getTestSuggestionObject(HYPO_USER_ACCOUNT);
        final Annotation annot = annotService.createAnnotation(jsAnnot, new UserInformation(theUser, Authorities.ISC));

        annotSuggestionService.acceptSuggestionById(annot.getId(), null);
    ...
    public void testAcceptSuggestionWithUserNotBeingGroupMember() throws Exception {
        ...
        final User user = userRepos.findByLoginAndContext(LOGIN, null);
        userGroupRepos.deleteAll(userGroupRepos.findByUserId(user.getId()));

        annotSuggestionService.acceptSuggestionById(annot.getId(), userInfo); // should work
