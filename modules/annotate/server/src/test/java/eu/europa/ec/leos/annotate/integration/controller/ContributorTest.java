...
public class ContributorTest {
     ...
     */
    @Test
    public void testSendAnnotationsAndCheckVisibilityForGroupUser() throws JsonProcessingException {
    ...
    /**
     *  send the annotations and verify that the contributor cannot see his own annotation when 
     *  asking for SENT annotations 
     */
    @Test
    public void testSendAnnotationsAndCheckVisibilityForContributor() throws JsonProcessingException {
        ...
        // the contributor sees only the public only of the standard SG user since his item 
        // is private and thus filtered out (it is still private as the contribution was not published)
        final List<JsonAnnotation> annotsContrib = executeSearchRequest(contribToken, SG, "", true);
        Assert.assertEquals(1, annotsContrib.size());
        verifyFoundAnnots(annotsContrib, Arrays.asList(idAnnot_SG));
    }
