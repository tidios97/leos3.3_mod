...
public class AnnotationBulkDeleteTest {
    ...
    public void testBulkDeleteAnnotations() throws Exception {
        ...
        final List<String> deletedAnnotations = annotService.deleteAnnotationsById(Arrays.asList(annot.getId(), annot2.getId()), userInfo);
        Assert.assertNotNull(deletedAnnotations);
        Assert.assertEquals(2, deletedAnnotations.size());
        Assert.assertTrue(deletedAnnotations.stream().anyMatch(annotId -> annotId.equals(annot.getId())));
        Assert.assertTrue(deletedAnnotations.stream().anyMatch(annotId -> annotId.equals(annot2.getId())));
        ...
        TestHelper.assertHasStatus(annotRepos, annot2.getId(), AnnotationStatus.DELETED, theUser.getId(), null);
        TestHelper.assertHasStatus(annotRepos, annot3.getId(), AnnotationStatus.NORMAL, null, null);
    }
    ...
    public void testBulkDeleteAnnotations_DeleteRootBeforeReply() throws Exception {
        ...
        final JsonAnnotation jsAnnotReply = TestData.getTestReplyToAnnotation(hypothesisUserAccount, new URI(annot.getDocument().getUri()), annot);
        final Annotation annotReply = annotService.createAnnotation(jsAnnotReply, userInfo);
        
        final JsonAnnotation jsAnnotReplyReply = TestData.getTestReplyToAnnotation(hypothesisUserAccount, new URI(annot.getDocument().getUri()),
                Arrays.asList(annot.getId(), annotReply.getId()));
        final Annotation annotReplyReply = annotService.createAnnotation(jsAnnotReplyReply, userInfo);

        Assert.assertEquals(3, annotRepos.count());
        Assert.assertEquals(1, documentRepos.count());

        // remove the root and the reply: by removing the root, all children should already be removed
        final List<String> deletedAnnotations = annotService.deleteAnnotationsById(Arrays.asList(annot.getId(), annotReply.getId()), userInfo);
        Assert.assertNotNull(deletedAnnotations);
        ...
        TestHelper.assertHasStatus(annotRepos, annot.getId(), AnnotationStatus.DELETED, theUser.getId(), null);
        TestHelper.assertHasStatus(annotRepos, annotReply.getId(), AnnotationStatus.DELETED, theUser.getId(), null);
        TestHelper.assertHasStatus(annotRepos, annotReplyReply.getId(), AnnotationStatus.DELETED, theUser.getId(), null);
    }
    ...
    public void testBulkDeleteAnnotations_NoneFound() throws Exception {
        ...
        final List<String> deletedAnnotations = annotService.deleteAnnotationsById(Arrays.asList(annot.getId() + "x", annot2.getId() + "y"), userInfo);
        Assert.assertNotNull(deletedAnnotations);
        ...
        TestHelper.assertHasStatus(annotRepos, annot2.getId(), AnnotationStatus.NORMAL, null, null);
    }
    ...
    public void testBulkDeleteAnnotations_NoUserinfo() {
