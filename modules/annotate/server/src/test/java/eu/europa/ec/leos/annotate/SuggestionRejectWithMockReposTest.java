...
/**
 * try to reject a suggestion, but repository throws exception -> appropriate exception should be given
 */
@RunWith(SpringRunner.class)
@SpringBootTest(properties = "spring.config.name=anot")
@ActiveProfiles("test")
public class SuggestionRejectWithMockReposTest {
    ...
    /**
     * a suggestion should be rejected, but repository throws exception
     * -> should throw expected exception
     */
    @Test(expected = CannotRejectSuggestionException.class)
    @SuppressFBWarnings(value = SpotBugsAnnotations.FieldNotInitialized, justification = SpotBugsAnnotations.FieldNotInitializedReason)
    public void testRejectSimpleSuggestionWithException() throws Exception {
        ...
        Mockito.when(userService.findByLoginAndContext(LOGIN, null)).thenReturn(theUser);
        ...
        ann.setId("1");
        ann.setUpdated(LocalDateTime.now(java.time.ZoneOffset.UTC));
        ...
        Mockito.when(annotPermService.hasUserPermissionToRejectSuggestion(Mockito.any(Annotation.class), Mockito.any(User.class))).thenReturn(true);
