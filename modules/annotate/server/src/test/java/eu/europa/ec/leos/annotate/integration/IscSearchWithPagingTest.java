...
public class IscSearchWithPagingTest {
    ...
    private void createTestData() {
        ...
        metaHelpInPrepV2.put(Metadata.PROP_RESPONSE_STATUS, ResponseStatus.IN_PREPARATION.toString());

        final Metadata metaSent = new Metadata(document, groupDigit, Authorities.ISC);
        MetadataHandler.setKeyValuePropertyFromSimpleMetadata(metaSent, metaHelpSentV1);
        addResponseId(metaSent, DIGIT);
        metadataRepos.save(metaSent);

        final Metadata metaInPrep = new Metadata(document, groupDigit, Authorities.ISC);
        MetadataHandler.setKeyValuePropertyFromSimpleMetadata(metaInPrep, metaHelpInPrepV2);
        ...
        annotRepos.saveAll(Arrays.asList(annotB, annotC, annotE, annotF, annotG, annotH));
    }
    ...
    private Annotation createAnnotation(final String annotId, final User user, final Metadata meta, final int minBeforeNow) {
        ...
        // mandatory fields
        annot.setId(annotId);
        annot.setCreated(LocalDateTime.now(java.time.ZoneOffset.UTC).minusMinutes(minBeforeNow));
