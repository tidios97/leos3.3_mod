...
public class IscSearchScenarioDigitTest {
    ...
    private void prepareDb() {
        ...
        groupRepos.saveAll(Arrays.asList(groupAgri, groupSg, groupSj, groupDigit));
        ...
        userRepos.saveAll(Arrays.asList(agriUser, sgUser, sjUser));
        ...
        userDetailsCache.cache(agriUser.getLogin(), null, new UserDetails(agriUser.getLogin(), Long.valueOf(2), AGRI, "user2", entitiesAgri, "", null));
        userDetailsCache.cache(sgUser.getLogin(), null, new UserDetails(sgUser.getLogin(), Long.valueOf(3), SG, "user3", entitiesSg, "", null));
        userDetailsCache.cache(sjUser.getLogin(), null, new UserDetails(sjUser.getLogin(), Long.valueOf(4), SJ, "user4", entitiesSj, "", null));
        ...
        tokenRepos.saveAll(Arrays.asList(agriToken, sgToken, sjToken, editToken));
    }
    ...
    private void executeStep4() throws CannotUpdateAnnotationException, MissingPermissionException, CannotUpdateSentAnnotationException {
        ...
        final Annotation annotSg1 = annotService.findAnnotationById(idAnnot_SG_1, sgUser.getLogin(), sgUser.getContext());
        ...
        jsAnnotSg1.setUpdated(LocalDateTime.now(java.time.ZoneOffset.UTC));
    ...
    public void testStep1Sg_Context1() throws JsonProcessingException {
        ...
        final String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(1, SG)));
    ...
    public void testStep3Agri_context2() throws JsonProcessingException {
        ...
        // consult SG's V1 answer
        final String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(1, SG, true)));
    ...
    public void testStep3Sj_context3() throws JsonProcessingException {
        ...
        final String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(1, SG, true),
                new MetadataRequest(1, SJ, false)));
    ...
    public void testStep4Sg_context1()
        ...
        // and SG's V1 answer
        final String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(1, SG),
                new MetadataRequest(2, SG, false)));
    ...
    public void testStep5Agri_context2()
        ...
        final String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(2, SG),
                new MetadataRequest(1, SG)));
    ...
    public void testStep6Sg_context1()
        ...
        final String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(3, SG, false),
                new MetadataRequest(1, SG),
                new MetadataRequest(2, SG)));
    ...
    public void testStep7Agri_context2()
        ...
        final String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(3, SG),
                new MetadataRequest(2, SG),
                new MetadataRequest(1, SG)));
    ...
    public void testStep7Sg_context2_deleteAgainAfterStep7()
        ...
        final String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(4, SG),
                new MetadataRequest(3, SG),
                new MetadataRequest(2, SG),
                new MetadataRequest(1, SG)));
    ...
    // test added in May 2020:
    // first five steps remain as-is
    // in step 6, SG creates two new annotations Annot_SG_3 and Annot_SG_4, but deletes one again
    // in step 7, the answer is SENT
    // -> verify that only one of the new annotations is returned, the other one shall not be reported
    // (since it was deleted before being sent, i.e. while answer was IN_PREPARATION)
    @Test
    public void testStep7Sg_context2_createTwoAndDeleteOneBeforeSendingInStep7()
            throws JsonProcessingException, CannotUpdateAnnotationException, MissingPermissionException, CannotUpdateSentAnnotationException,
            CannotDeleteAnnotationException, CannotDeleteSentAnnotationException {
        ...
        // create two new annotations
        final JsonAnnotation jsAnnotSg3 = createAnnotation("ANNOT_SG_3", sgUser, groupDigit, SG, 3, sgToken); // posted to DIGIT!
        final String idAnnot_SG_3 = jsAnnotSg3.getId();

        final JsonAnnotation jsAnnotSg4 = createAnnotation("ANNOT_SG_4", sgUser, groupDigit, SG, 3, sgToken); // posted to DIGIT!
        final String idAnnot_SG_4 = jsAnnotSg4.getId();

        // delete the first one again (Annot_SG_3)
        final UserInformation sgUserInfo = new UserInformation(sgToken);
        annotService.deleteAnnotationById(idAnnot_SG_3, sgUserInfo);

        // first verification
        // consult SG's V3 answer (IN_PREPARATION and previous SENT answers V2 and V1)
        String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(3, SG, false),
                new MetadataRequest(2, SG),
                new MetadataRequest(1, SG)));

        List<JsonAnnotation> foundAnnots = executeSearchRequest(sgToken, SG, serialisedMetadata);
        verifyFoundAnnots(foundAnnots, Arrays.asList(idAnnot_SG_1, idAnnot_SG_1_changed, idAnnot_SG_2, idAnnot_SG_4));
        // Annot_SG_1_changed and Annot_SG_2 are visible since we ask for all version and statuses
        // Annot_SG_3 is NOT visible since we ask for NORMAL status, but it has DELETED status
        // Annot_SG_4 is visible since we ask for NORMAL status

        executeStep7(); // sends it

        // second verification
        // consult SG's V3 answer (including V2 and V1)
        serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(3, SG),
                new MetadataRequest(2, SG),
                new MetadataRequest(1, SG)));

        foundAnnots = executeSearchRequest(sgToken, SG, serialisedMetadata);
        verifyFoundAnnots(foundAnnots, Arrays.asList(idAnnot_SG_1, idAnnot_SG_1_changed, idAnnot_SG_2, idAnnot_SG_4));
        // Annot_SG_1_changed and Annot_SG_2 are visible since we ask for all version and statuses
        // Annot_SG_3 is NOT visible: it is filtered out since it has DELETED status, but sentDeleted=false
        // Annot_SG_4 is visible since we ask for NORMAL status
    ...
    public void testStep8Agri_context3()
        ...
        final String serialisedMetadata = getSearchMetadata(Arrays.asList(new MetadataRequest(1, SJ)));
    ...
    private static class MetadataRequest {
        ...
        public MetadataRequest(final int respVers, final String respId, final boolean allStats) {
            this.responseVersion = respVers;
            this.responseId = respId;
            this.allStatuses = allStats;
        }

        public MetadataRequest(final int respVers, final String respId) {
            this.responseVersion = respVers;
            this.responseId = respId;
    ...
    private String getSearchMetadata(final List<MetadataRequest> metaPairs) throws JsonProcessingException {

        final List<SimpleMetadataWithStatuses> requestedMetadata = new ArrayList<SimpleMetadataWithStatuses>();

        for (final MetadataRequest metaReq : metaPairs) {

            final SimpleMetadata metaHelp = getMetadataForQuery(metaReq.getResponseVersion(), metaReq.getResponseId());
            final AnnotationStatus status = metaReq.isAllStatuses() ? AnnotationStatus.ALL : AnnotationStatus.NORMAL;
            requestedMetadata.add(new SimpleMetadataWithStatuses(metaHelp, Arrays.asList(status)));
        }
    ...
    private void verifySentDeletedAndDeleted(final String annotId, final long sentDeletedVersion) {
        ...
        final Annotation annot = annotRepos.findById(annotId).orElse(null);
