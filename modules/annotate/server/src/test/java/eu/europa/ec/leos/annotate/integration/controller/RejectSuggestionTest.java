...
public class RejectSuggestionTest {
    ...
    public void testRejectSuggestionOk() throws Exception {
        ...
        final Annotation annot = annotService.createAnnotation(jsAnnot, new UserInformation(user, Authorities.ISC));
        ...
        TestHelper.assertHasStatus(annotRepos, annotId, AnnotationStatus.REJECTED, user.getId(), null);
    }
    ...
    public void testRejectSuggestionNotOkIsNoSuggestion() throws Exception {
        ...
        final Annotation annot = annotService.createAnnotation(jsAnnot, new UserInformation(user, Authorities.EdiT));
        final String annotId = annot.getId();
    ...
    /**
     * try rejecting an existing suggestion, is accepted (since ANOT-135) although user is not member
     * of the respective group in which the annotation was published
     * expected HTTP 200
     */
    @SuppressFBWarnings(value = SpotBugsAnnotations.FieldNotInitialized, justification = SpotBugsAnnotations.FieldNotInitializedReason)
    @Test
    public void testRejectSuggestionOkForNonGroupMembers() throws Exception {
        ...
        // the annotation was rejected
        Assert.assertEquals(1, annotRepos.count());
        TestHelper.assertHasStatus(annotRepos, annotId, AnnotationStatus.REJECTED, secondUser.getId(), null);
    }
