...
public class AnnotationDeleteTest {
    ...
    public void testDeleteSimpleAnnotation() throws Exception {
        ...
        userDetailsCache.cache(LOGIN, null, new UserDetails(LOGIN, theUser.getId(), "use", "r", Arrays.asList(new UserEntity("1", "EMPL", "EMPL")),
                "use@r.com", null));
        ...
        Assert.assertEquals(1, documentRepos.count());
        Assert.assertEquals(2, tagRepos.count());
        TestHelper.assertHasStatus(annotRepos, annot.getId(), AnnotationStatus.DELETED, theUser.getId(), null);

        // check that the annotation status would be reported correctly in JSON format
        final Annotation annotRetrieved = annotRepos.findById(annot.getId()).get();
        final JsonAnnotation jsConverted = conversionService.convertToJsonAnnotation(annotRetrieved, userInfo);
    ...
    public void testDeleteAnnotationWithoutId() throws Exception {
        ...
        annotService.deleteAnnotationById("", new UserInformation(LOGIN, null, Authorities.EdiT));
    }
    ...
    public void testDeleteSimpleAnnotationOfOtherUser() throws Exception {
        ...
        final UserInformation userInfo = new UserInformation(LOGIN, null, authority);
        userInfo.setUser(theUser);
        final UserInformation otherUserInfo = new UserInformation(otherLogin, null, authority);
        ...
        annotService.deleteAnnotationById(annot.getId(), otherUserInfo);

        // verify that the tags and the document have not been removed, but the annotation is flagged as being deleted
        Assert.assertEquals(1, annotRepos.count());
        Assert.assertEquals(1, documentRepos.count());
        Assert.assertEquals(2, tagRepos.count());
        TestHelper.assertHasStatus(annotRepos, annot.getId(), AnnotationStatus.DELETED, otherUser.getId(), null);
    }
    ...
    public void testSentDeleteSentAnnotationFromIsc() throws Exception {
        ...
        final Metadata savedMetadata = annotService.findAnnotationById(annot.getId()).getMetadata();
        ...
        final Annotation readAnnot = annotRepos.findById(annot.getId()).orElse(null);
    ...
    public void testSentDeleteSentAnnotationFromIscTwiceDoesNotToggleStatus() throws Exception {
        ...
        // verify: annotation has the "sentDeleted" flag
        Annotation readAnnot = annotRepos.findById(annot.getId()).orElse(null);
        Assert.assertNotNull(readAnnot);
        Assert.assertTrue(readAnnot.isSentDeleted());
        ...
        readAnnot = annotRepos.findById(annot.getId()).get();
        Assert.assertTrue(readAnnot.isSentDeleted());
    }
    ...
    public void testSentDeleteSentAnnotationFromIscSameGroup() throws Exception {
        ...
        final UserInformation userInfo = new UserInformation(username, null, authority);
    ...
    public void testDeleteSentAnnotationFromEdit() throws Exception {
        ...
        final UserInformation userInfoEdit = new UserInformation(LOGIN, null, Authorities.EdiT);
        annotService.deleteAnnotationById(annot.getId(), userInfoEdit);
    ...
    public void testDeleteSimpleReplyOfOtherUser() throws Exception {
        ...
        final Annotation annot = annotService.createAnnotation(jsAnnot, userInfo);
        final JsonAnnotation jsReply = TestData.getTestReplyToAnnotation(LOGIN, new URI(annot.getDocument().getUri()), Arrays.asList(annot.getId()));
        final Annotation reply = annotService.createAnnotation(jsReply, userInfo);

        Assert.assertEquals(2, annotRepos.count());

        // other user tries to remove the reply -> ok
        annotService.deleteAnnotationById(reply.getId(), otherUserInfo);
        ...
        TestHelper.assertHasStatus(annotRepos, annot.getId(), AnnotationStatus.NORMAL, null, null);
        TestHelper.assertHasStatus(annotRepos, reply.getId(), AnnotationStatus.DELETED, otherUser.getId(), null);
    }
    ...
    public void testSentDeleteSentReplyFromIsc() throws Exception {
        ...
        final Metadata savedMetadata = annotService.findAnnotationById(reply.getId()).getMetadata();
        savedMetadata.setResponseStatus(ResponseStatus.SENT);
        metadataRepos.save(savedMetadata);

        // try to remove the reply -> will be flagged
        annotService.deleteAnnotationById(reply.getId(), userInfo);
        ...
        TestHelper.assertHasStatus(annotRepos, reply.getId(), AnnotationStatus.NORMAL, null, null);
        Assert.assertTrue(annotRepos.findById(reply.getId()).get().isSentDeleted());
    }
    ...
    public void testDeleteSentReplyFromLeos() throws Exception {
        ...
        TestHelper.assertHasStatus(annotRepos, reply.getId(), AnnotationStatus.DELETED, userInfo.getUser().getId(), null);
    }
    ...
    public void testDeleteOneAnnotationOfTwoForSameDocument() throws Exception {
        ...
        final Annotation annotFirst = annotService.createAnnotation(jsAnnotFirst, userInfo);
        ...
        final String annotId1 = annotFirst.getId();
        final String annotId2 = annotSecond.getId();
        ...
        TestHelper.assertHasStatus(annotRepos, annotId1, AnnotationStatus.DELETED, theUser.getId(), null);
        TestHelper.assertHasStatus(annotRepos, annotId2, AnnotationStatus.NORMAL, null, null);
    }
    ...
    public void testEntireAnnotationThreadRemoved() throws Exception {
        ...
        final JsonAnnotation jsAnnotFirstReply = TestData.getTestReplyToAnnotation(hypothesisUserAccount, jsAnnot.getUri(), Arrays.asList(annotId));
        final Annotation annotFirstReply = annotService.createAnnotation(jsAnnotFirstReply, userInfo);
        Assert.assertEquals(2, annotRepos.count());

        // add a reply to the reply
        final JsonAnnotation jsAnnotFirstReplyReply = TestData.getTestReplyToAnnotation(hypothesisUserAccount, jsAnnot.getUri(),
                Arrays.asList(annotId, annotFirstReply.getId()));
        final Annotation annotFirstReplyReply = annotService.createAnnotation(jsAnnotFirstReplyReply, userInfo);
        Assert.assertEquals(3, annotRepos.count());

        // create a second reply to the root annotation
        final JsonAnnotation jsAnnotSecondReply = TestData.getTestReplyToAnnotation(hypothesisUserAccount, jsAnnot.getUri(), Arrays.asList(annotId));
        final Annotation annotSecondReply = annotService.createAnnotation(jsAnnotSecondReply, userInfo);
        ...
        // now remove the root
        annotService.deleteAnnotationById(annotId, userInfo);
        ...
        Assert.assertEquals(4, annotRepos.count());
        Assert.assertEquals(1, documentRepos.count());
        assertFlag(Arrays.asList(annot, annotFirstReply, annotFirstReplyReply, annotSecondReply), AnnotationStatus.DELETED, theUser.getId());
    }
    ...
    public void testDeleteReplyWithoutSubreply() throws Exception {
        ...
        // retrieve out test annotation: has two tags; save it
        final JsonAnnotation jsAnnot = TestData.getTestAnnotationObject(hypothesisUserAccount);
        final UserInformation userInfo = new UserInformation(LOGIN, null, auth);
        userInfo.setUser(theUser);
        ...
        final JsonAnnotation jsAnnotSecondReply = TestData.getTestReplyToAnnotation(hypothesisUserAccount, jsAnnot.getUri(), Arrays.asList(annotId));
        final Annotation annotSecondReply = annotService.createAnnotation(jsAnnotSecondReply, userInfo);
        final String annotSecondReplyId = annotSecondReply.getId();
        Assert.assertEquals(4, annotRepos.count());
        ...
        annotService.deleteAnnotationById(annotSecondReplyId, userInfo);
        ...
        Assert.assertEquals(1, documentRepos.count());
        TestHelper.assertHasStatus(annotRepos, annotSecondReplyId, AnnotationStatus.DELETED, theUser.getId(), null);

        // other annotations are not flagged
        assertFlag(Arrays.asList(annot, annotFirstReply, annotFirstReplyReply), AnnotationStatus.NORMAL, null);
    }
    ...
    public void testDeleteReplyHavingSubreply() throws Exception {
        ...
        final JsonAnnotation jsAnnotFirstReply = TestData.getTestReplyToAnnotation(hypothesisUserAccount, jsAnnot.getUri(), Arrays.asList(annotId));
        final Annotation annotFirstReply = annotService.createAnnotation(jsAnnotFirstReply, userInfo);
        final String annotFirstReplyId = annotFirstReply.getId();
        Assert.assertEquals(2, annotRepos.count());

        // add a reply to the reply
        final JsonAnnotation jsAnnotFirstReplyReply = TestData.getTestReplyToAnnotation(hypothesisUserAccount, jsAnnot.getUri(),
                Arrays.asList(annotId, annotFirstReplyId));
        final Annotation annotFirstReplyReply = annotService.createAnnotation(jsAnnotFirstReplyReply, userInfo);
        Assert.assertEquals(3, annotRepos.count());
        ...
        annotService.deleteAnnotationById(annotFirstReplyId, userInfo);
        ...
        TestHelper.assertHasStatus(annotRepos, annotFirstReplyId, AnnotationStatus.DELETED, theUser.getId(), null);

        // verify in particular that the annotation, the sub reply of first reply, and the second reply are the items remaining
        assertFlag(Arrays.asList(annot, annotFirstReplyReply, annotSecondReply), AnnotationStatus.NORMAL, null);
        Assert.assertNotNull(annotService.findAnnotationById(annotId)); // annotation
        Assert.assertNotNull(annotService.findAnnotationById(annotFirstReplyReply.getId())); // sub reply of first reply
        Assert.assertNotNull(annotService.findAnnotationById(annotSecondReply.getId())); // second reply
    }

    private void assertFlag(final List<Annotation> annots, final AnnotationStatus status, final Long userId) {
        annots.forEach(annot -> TestHelper.assertHasStatus(annotRepos, annot.getId(), status, userId, null));
    }
