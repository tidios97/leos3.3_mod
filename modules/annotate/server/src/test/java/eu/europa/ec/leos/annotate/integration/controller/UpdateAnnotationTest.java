...
public class UpdateAnnotationTest {
    ...
    private static final String ACCT = "acct:user@";
    ...
    public void testUpdateAnnotationOk() throws Exception {
        ...
        final String UPDATED_TEXT = "updated text";
        final String UPDATED_TAG = "updatedtag";
        final String authority = Authorities.ISC;
        final String hypothesisUserAccount = ACCT + authority;
        ...
        jsAnnotCreated.setText(UPDATED_TEXT);
        jsAnnotCreated.setTags(Arrays.asList(UPDATED_TAG));

        // send
        final String serializedAnnotation = SerialisationHelper.serialize(jsAnnotCreated);
        ...
        final Annotation ann = annotRepos.findById(annotId).orElse(null);
    ...
    /**
     * successfully update an annotation with a connected entity specified, 
     * expected HTTP 200 and returned annotation
     */
    @SuppressFBWarnings(value = SpotBugsAnnotations.FieldNotInitialized, justification = SpotBugsAnnotations.FieldNotInitializedReason)
    @Test
    public void testUpdateAnnotationOkWithConnectedEntity() throws Exception {

        final String ENTITY = "SomeEntity";
        ...
        // create a new group (entity) and assign user to it
        final Group newGroup = new Group(ENTITY, false);
        groupRepos.save(newGroup);
        userGroupRepos.save(new UserGroup(user.getId(), newGroup.getId()));
        ...
        final MockHttpServletRequestBuilder builder = MockMvcRequestBuilders
                .patch(API_URL + annotId + "?connectedEntity=" + ENTITY)
                .contentType(MediaType.APPLICATION_JSON)
                .content(serializedAnnotation)
                .header(TestHelper.AUTH_HEADER, TestHelper.AUTH_BEARER + ACCESS_TOKEN);
        ...
        // check the content was updated in the database, especially the connected entity
        final Annotation ann = annotRepos.findById(annotId).orElse(null);
        Assert.assertNotNull(ann);
        Assert.assertEquals(UPDATED_TEXT, ann.getText());
        Assert.assertEquals(newGroup.getId(), ann.getConnectedEntityId());
    }
    ...
    public void testUpdateSentAnnotationFailure() throws Exception {
        ...
        // modify the annotation
        final JsonAnnotation jsAnnotCreated = conversionService.convertToJsonAnnotation(annot, userInfo);
        jsAnnotCreated.setText(initialText + " new text");
    ...
    public void testUpdateSentAnnotationOk() throws Exception {
        ...
        final Group group = groupRepos.findByName(annot.getGroup().getName());
        userGroupRepos.save(new UserGroup(user.getId(), group.getId()));
        ...
        final JsonAnnotation jsResponse = SerialisationHelper.deserializeJsonAnnotation(responseString);
        Assert.assertNotNull(jsResponse);
        Assert.assertNotEquals(jsResponse.getId(), annotId);
        Assert.assertEquals(jsResponse.getLinkedAnnotationId(), annotId);
        Assert.assertTrue(jsResponse.getDocument().getMetadata().containsValue(ResponseStatus.IN_PREPARATION.toString()));
    ...
    /**
     * successfully update a forwarded annotation's forwarding justification (add text, previously empty), 
     * expected HTTP 200 and returned annotation
     */
    @SuppressFBWarnings(value = SpotBugsAnnotations.FieldNotInitialized, justification = SpotBugsAnnotations.FieldNotInitializedReason)
    @Test
    public void testUpdateForwardedAnnotationOk() throws Exception {

        final String UPDATED_FWD_JUST_TEXT = "updated text";
        final String authority = Authorities.ISC;
        final String hypothesisUserAccount = ACCT + authority;
        final Group originGroup = groupRepos.save(new Group("Test", true));
        final UserInformation userInfo = new UserInformation(user, authority);

        // preparation: save an annotation that can be updated later on
        final JsonAnnotation jsAnnot = TestData.getTestAnnotationObject(hypothesisUserAccount);
        jsAnnot.setForwarded(true);
        jsAnnot.setForwardedJustification(null); // no comment initially
        ...
        // modify the annotation
        final JsonAnnotation jsAnnotCreated = conversionService.convertToJsonAnnotation(annot, userInfo);
        jsAnnotCreated.setForwardedJustification(UPDATED_FWD_JUST_TEXT);
        ...
        // forward justification must have been set, forwarding flag still present
        final JsonAnnotation jsResponse = SerialisationHelper.deserializeJsonAnnotation(responseString);
        Assert.assertNotNull(jsResponse);
        Assert.assertEquals(annotId, jsResponse.getId());
        Assert.assertEquals(UPDATED_FWD_JUST_TEXT, jsResponse.getForwardJustification());
        ...
        final Annotation ann = annotRepos.findById(annotId).orElse(null);
        Assert.assertNotNull(ann);
        Assert.assertEquals(UPDATED_FWD_JUST_TEXT, ann.getForwardJustification());
        Assert.assertEquals(originGroup, ann.getOriginGroup());
    }

    /**
     * successfully update a forwarded annotation's forwarding justification (remove it, previously set);
     * also try to remove the forwarding flag, but should not be changed in the database
     * expected HTTP 200 and returned annotation
     */
    @SuppressFBWarnings(value = SpotBugsAnnotations.FieldNotInitialized, justification = SpotBugsAnnotations.FieldNotInitializedReason)
    @Test
    public void testUpdateForwardedAnnotation2Ok() throws Exception {

        final String INITIAL_FWD_JUST_TEXT = "forwarding text";
        final String authority = Authorities.ISC;
        final String hypothesisUserAccount = ACCT + authority;
        final Group originGroup = groupRepos.save(new Group("Test", true));
        ...
        final JsonAnnotation jsAnnot = TestData.getTestAnnotationObject(hypothesisUserAccount);
        jsAnnot.setForwarded(true);
        jsAnnot.setForwardedJustification(INITIAL_FWD_JUST_TEXT);
        jsAnnot.setOriginGroup(originGroup.getName());
        final Group group = groupRepos.findByName(jsAnnot.getGroup());
        userGroupRepos.save(new UserGroup(user.getId(), group.getId()));
        final Annotation annot = annotService.createAnnotation(jsAnnot, userInfo);
        ...
        // modify the annotation
        final JsonAnnotation jsAnnotCreated = conversionService.convertToJsonAnnotation(annot, userInfo);
        jsAnnotCreated.setForwardedJustification(null); // text removed
        jsAnnotCreated.setForwarded(false); // pretend to remove the forwarding flag, should be ignored!
        ...
        Assert.assertNotNull(jsResponse);
        Assert.assertEquals(annotId, jsResponse.getId());
        Assert.assertNull(jsResponse.getForwardJustification()); // removed
        Assert.assertTrue(jsResponse.isForwarded()); // remains set!
        ...
        final Annotation ann = annotRepos.findById(annotId).orElse(null);
        Assert.assertNotNull(ann);
        Assert.assertTrue(ann.isForwarded());
        Assert.assertNull(ann.getForwardJustification());
        Assert.assertEquals(ann.getOriginGroup(), originGroup);
    }
