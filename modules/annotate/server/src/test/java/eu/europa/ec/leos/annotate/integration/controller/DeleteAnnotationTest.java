...
public class DeleteAnnotationTest {
    ...
    public void testDeleteAnnotationOk() throws Exception {
        ...
        final Annotation annot = annotService.createAnnotation(jsAnnot, new UserInformation(user, "AGRI"));
        ...
        TestHelper.assertHasStatus(annotRepos, annotId, AnnotationStatus.DELETED, user.getId(), null);
    }
    ...
    public void testDeleteAnnotationOfAnotherUser() throws Exception {
        ...
        final JsonAnnotation jsAnnot = TestData.getTestAnnotationObject(hypothesisUserAccount);
        final Annotation annot = annotService.createAnnotation(jsAnnot, new UserInformation(user, authority));
        final String annotId = annot.getId();
        Assert.assertEquals(AnnotationStatus.NORMAL, annotRepos.findById(annotId).get().getStatus()); // check: annotation is existing
        ...
        // the annotation was deleted
        Assert.assertEquals(1, annotRepos.count());
        Assert.assertEquals(AnnotationStatus.DELETED, annotRepos.findById(annotId).get().getStatus()); // check: annotation is flagged as being deleted
    }
    ...
    public void testDeleteSentAnnotationForIscUserOk() throws Exception {
        ...
        Assert.assertEquals(1, annotRepos.count());
        TestHelper.assertHasStatus(annotRepos, annotId, AnnotationStatus.NORMAL, null, null);
        Assert.assertTrue(annotRepos.findById(annotId).get().isSentDeleted());
    }
}
