...
import eu.europa.ec.leos.annotate.websockets.MessageBroker.ACTION;
...
public class MessageBrokerTest {
    ...
    public void testBroadcast_Annotation_is_anonymised() throws Exception {
        ...
        final WebSocketSession subscriber = Mockito.mock(WebSocketSession.class);
        final UserInformation userInformation = new UserInformation("login", null, Authorities.EdiT); // subscribing user is EdiT user
        userInformation.setClientId("x1");
        userInformation.setUserDetails(new UserDetails("login", 8L, "f", "n", null, "", Collections.singletonList("author")));
        final WebSocketSession subscriber2 = Mockito.mock(WebSocketSession.class);
        final UserInformation userInformation2 = Mockito.mock(UserInformation.class);
        Mockito.when(userInformation2.getClientId()).thenReturn("x2");
    ...
    private String getId(final TextMessage textMessage) throws IOException {
        final ObjectMapper objMapper = new ObjectMapper();
        final Map<String, Object> message = objMapper.readValue(textMessage.getPayload(), Map.class);
        final Map<String, Object> annotation = ((ArrayList<Map<String, Object>>) (message.get("payload"))).get(0);
        return (String) annotation.get("id");
    }
    
    @SuppressWarnings("unchecked")
    private Map<String, String> getUserInfo(final TextMessage textMessage) throws IOException {
        final ObjectMapper objMapper = new ObjectMapper();
        final Map<String, Object> message = objMapper.readValue(textMessage.getPayload(), Map.class);
        final Map<String, Object> annotation = ((ArrayList<Map<String, Object>>) (message.get("payload"))).get(0);
