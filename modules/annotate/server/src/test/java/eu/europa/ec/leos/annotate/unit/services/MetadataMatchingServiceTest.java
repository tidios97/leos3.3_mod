...
public class MetadataMatchingServiceTest {
    ...
    public void testAllMetadataContainedInDbObject_Successful() {
        ...
        kvPairs.put(Metadata.PROP_RESPONSE_STATUS, ResponseStatus.SENT.toString());
        ...
        MetadataHandler.setKeyValuePropertyFromSimpleMetadata(dbMetadata, kvPairs);
    ...
    public void testGetIdsOfMatchingMetadatas() {
        ...
        MetadataHandler.setKeyValuePropertyFromSimpleMetadata(firstMeta, itemsFirstMeta);
        ...
        MetadataHandler.setKeyValuePropertyFromSimpleMetadata(secondMeta, itemsSecondMeta);
        ...
        MetadataHandler.setKeyValuePropertyFromSimpleMetadata(thirdMeta, itemsThirdMeta);
    ...
    public void testHighestResponseVersion_inPreparation() {
        ...
        metaSent.setResponseStatus(ResponseStatus.SENT);
        ...
        metaInPrep.setResponseStatus(ResponseStatus.IN_PREPARATION);
    ...
    public void testHighestResponseVersion_sentAndDeleted() {
        ...
        annotSent.setCreated(LocalDateTime.now(java.time.ZoneOffset.UTC));
        annotSent.setUpdated(LocalDateTime.now(java.time.ZoneOffset.UTC));
    ...
    // check that no identical metadata is found
    @Test
    public void testFindInPrepItem_nothingFound() {

        final Metadata annotMeta = new Metadata();
        annotMeta.setSystemId(Authorities.ISC);

        final Document doc = new Document();
        doc.setTitle("helloDoc");
        doc.setUri("http://a");
        documentRepos.save(doc);
        annotMeta.setDocument(doc);

        final Group group = new Group("agroup", true);
        groupRepos.save(group);
        annotMeta.setGroup(group);

        metadataRepos.save(annotMeta);

        final Annotation annot = new Annotation();
        annot.setMetadata(annotMeta);
        annotMeta.setKeyValuePairs("something:anything"); // modify the metadata such that no match is found

        final Metadata newMeta = metadataMatchingService.findOrCreateInPrepItemForAnnotToDelete(annot);
        Assert.assertNull(newMeta);
    }
