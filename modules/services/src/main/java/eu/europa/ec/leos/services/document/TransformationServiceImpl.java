...
public class TransformationServiceImpl implements TransformationService {
    ...
    public String formatToHtml(XmlDocument versionDocument, String contextPath, List<LeosPermission> permissions, InputStream coverPageStream) {
        ...
        try (final ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream
                (versionDocument.getContent().getOrError(() -> "Version document content is required!")
                        .getSource().getBytes())) {
            return transform(byteArrayInputStream, editableXHtmlTemplate, contextPath, permissions, coverPageStream);
        } catch (Exception e) {
    ...
    public String formatToHtml(XmlDocument versionDocument, String contextPath, List<LeosPermission> permissions) {
        ...
        try (final ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream
                (versionDocument.getContent().getOrError(() -> "Version document content is required!")
                        .getSource().getBytes())) {
            return transform(byteArrayInputStream, editableXHtmlTemplate, contextPath, permissions, null);
        } catch (Exception e) {
            throw new RuntimeException("Unable to format to HTML", e);
        }
    }
    ...
    private String transform(InputStream documentStream, String templateName, String contextPath, List<LeosPermission>
        ...
        try {
            ...
            if(coverPageStream != null && coverPageStream.available() > 0) {
                NodeModel coverPageModel = XmlNodeModelHandler.parseXmlStream(coverPageStream);
                root.put("cover_data", coverPageModel);
            }
