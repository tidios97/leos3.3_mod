...
public class ByteArrayBuilder {
    ...
    public void appendAtTheBeginning(byte... otherContent) {
        if (otherContent.length > 0) {
            byte[] joinedArray = new byte[content.length + otherContent.length];
            System.arraycopy(otherContent, 0, joinedArray, 0, otherContent.length);
            System.arraycopy(content, 0, joinedArray, otherContent.length, content.length);
