...
public class SecurityContext {
    ...
    /*
     * This method was created to check the permissions from an Authentication object that we can
     * create using the createUserWithAuthorities(String login) from UserServiceImpl.
     * The method hasPermission(Object domainObject, LeosPermission permission) checks only
     * permissions from user that is in the current thread.
     */
    public boolean hasPermission(Object domainObject, LeosPermission permission, Authentication authentication){
        return leosPermissionEvaluator.hasPermission(authentication, domainObject == null ? new Object() : domainObject,
                permission.name());
    }
