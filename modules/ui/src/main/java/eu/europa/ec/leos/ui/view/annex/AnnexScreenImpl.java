...
abstract class AnnexScreenImpl extends VerticalLayout implements AnnexScreen {
    ...
    protected HorizontalLayout annexToolBar;
    ...
    @Override
    public void setTitle(String title, String number) {
        StringBuilder combinedTitle = new StringBuilder();
        combinedTitle.append((number == null || number.isEmpty())
    ...
    void init() {
        ...
        new ChangeDetailsExtension<>(annexContent, eventBus);
    ...
    public void initSearchAndReplace() {
        searchDelegate = new SearchDelegate(searchButton, annexLayout, messageHelper, eventBus, annexContent, searchAndReplaceEnabled);
    }
    ...
    public void setPermissions(DocumentVO annex, boolean isClonedProposal){
        ...
        if(enableUpdate) {
            if(leosEditorExtension == null) {
                eventBus.post(new InitLeosEditorEvent(annex));
            }
            if(actionManagerExtension == null) {
                actionManagerExtension = new ActionManagerExtension<>(annexContent, instanceTypeResolver.getInstanceType(), eventBus, structureContextProvider.get().getTocItems());
            }
        }
    }
