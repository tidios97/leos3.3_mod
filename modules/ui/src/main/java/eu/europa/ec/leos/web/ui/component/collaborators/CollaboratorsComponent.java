...
public class CollaboratorsComponent extends CustomComponent {
    ...
    @Value("#{integrationProperties['econsilium.search.context']}")
    ...
    private HorizontalLayout searchContextHolder;
    ...
    public void populateData(List<CollaboratorVO> collaborators) {
        ...
        if (collaborators != null) {
            Comparator<CollaboratorVO> comparator = Comparator.<CollaboratorVO, String>comparing(o -> o.getUser().getName())
                    .thenComparing(o -> o.getUser().getEntity());
            List<CollaboratorVO> sortedCollaborators = new ArrayList<>(collaborators);
            sortedCollaborators.sort(comparator);
            sortedCollaborators.forEach(container::addItem);
        }
    }
    ...
    public void addCollaborator() {
        if (editor == null) {
            editor = new CollaboratorEditor(collaboratorGrid, messageHelper, eventBus, urlBuilder.getDocumentUrl(this.getUI().getPage()), authorityMapHelper, defaultSearchContext);
        }
