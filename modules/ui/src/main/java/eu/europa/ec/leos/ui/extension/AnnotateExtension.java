...
public class AnnotateExtension<T extends AbstractField<V>, V> extends LeosJavaScriptExtension {
    ...
    public AnnotateExtension(T target, EventBus eventBus, ConfigurationHelper cfgHelper, String containerId,
                             ...
                             String proposalRef, String connectedEntity) {
        super();
    ...
    private void registerServerSideAPI() {
        ...
        addFunction("requestMergeSuggestions", arguments -> {
            List<MergeSuggestionRequest> suggestionsObjectsList = new ArrayList<>();
            ...
            for (int i = 0; i <= jsonArray.length(); i++) {
                try {
                    ...
                    suggestionsObjectsList.add(new MergeSuggestionRequest(origText, newText, elementId, startOffset, endOffset));
                } catch (Exception e) {
                    LOG.debug("Request merge suggestion stopped because of bad arguments in the request");
                }
            }
            eventBus.post(new MergeSuggestionsRequest(suggestionsObjectsList));
        });
        addFunction("responseFilteredAnnotations", arguments -> {
            String annotations = arguments.getString(0);

            eventBus.post(new ResponseFilteredAnnotations(annotations));
        });
    ...
    @Subscribe
    public void resultMergeSuggestions(MergeSuggestionsResponse event) {
        List<JsonObject> suggestionsMergeResult = event.getSuggestionsResponseList();
        Object[] result = suggestionsMergeResult.toArray();
        callFunction("receiveMergeSuggestions", result);
    }
