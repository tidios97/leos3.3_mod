...
class CollaboratorEditor {
    ...
    CollaboratorEditor(GridWithEditorListener collaboratorGrid, MessageHelper messageHelper, EventBus eventBus, String documentURL, LeosPermissionAuthorityMapHelper authorityMapHelper, String defaultSearchContext) {
    ...
    private UserSearchComponent getUserEditor() {
        if(StringUtils.isNotEmpty(defaultSearchContext)){
            userSearchComponent = new UserSearchComponent(defaultSearchContext, messageHelper, eventBus);
        }else{
    ...
    private class CollaboratorCommitHandler implements FieldGroup.CommitHandler {
        ...
        public void postCommit(FieldGroup.CommitEvent commitEvent) throws FieldGroup.CommitException {
            try {
                ...
                collaboratorGrid.setSortOrder(Arrays.asList(
                        new SortOrder(COLUMN.NAME.getKey(), SortDirection.ASCENDING),
                        new SortOrder(COLUMN.ENTITY.getKey(), SortDirection.ASCENDING))
                );
            } finally {
    ...
    private class CollaboratorEditorListener implements Grid.EditorListener {
        ...
        public void editorOpened(Grid.EditorOpenEvent e) {
            ...
            if(StringUtils.isNotEmpty(defaultSearchContext)){ eventBus.post(new SearchContextEvent(true)); }
        ...
        public void editorClosed(Grid.EditorCloseEvent e) {
            ...
            if(StringUtils.isNotEmpty(defaultSearchContext)){ eventBus.post(new SearchContextEvent(false)); }
