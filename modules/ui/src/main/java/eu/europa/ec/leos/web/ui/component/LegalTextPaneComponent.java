...
public class LegalTextPaneComponent extends CustomComponent {
    ...
    private ContributionsTab<Bill> contributionTab;
    ...
    public LegalTextPaneComponent(EventBus eventBus, MessageHelper messageHelper, ConfigurationHelper cfgHelper, SecurityContext securityContext,
                                  LegalTextActionsMenuBar legalTextActionsMenuBar, TocEditor tocEditor, VersionsTab<Bill> versionsTab, ContributionsTab<Bill> contributionTab,
                                  Provider<StructureContext> structureContextProvider, UserHelper userHelper,
                                  TableOfContentProcessor tableOfContentProcessor, InstanceTypeResolver instanceTypeResolver) {
    ...
    public void initSearchAndReplace(boolean searchAndReplaceEnabled) {
        legalTextComponent.initSearchAndReplace(searchAndReplaceEnabled);
    }
    ...
    private void createChildComponents() {
        ...
        legalTextComponent = new LegalTextComponent(eventBus, messageHelper, cfgHelper, securityContext, legalTextActionMenuBar, structureContextProvider,
                userHelper, instanceTypeResolver);
        ...
        tableOfContentComponent = new TableOfContentComponent(messageHelper, eventBus, securityContext, cfgHelper, tocEditor,
                structureContextProvider, tableOfContentProcessor, indentListRadioButtonGroupItemsToEnable);
    ...
    public void initAnnotations(DocumentVO bill, String proposalRef, String connectedEntity) {
        legalTextComponent.initAnnotateExtension(bill, proposalRef, connectedEntity);
    }
    ...
    public void setDataFunctions(List<VersionVO> allVersions,
                                 ...
                                 boolean isComparisonAvailable, boolean isBaseVersionMenuAvailable,
                                 boolean canRestorePreviousVersion, boolean canDownload) {

        versionsTab.setDataFunctions(allVersions, minorVersionsFn, countMinorVersionsFn, recentChangesFn,
                countRecentChangesFn, isComparisonAvailable, isBaseVersionMenuAvailable,
                canRestorePreviousVersion, canDownload);

        if (!allContributions.isEmpty()) {
            accordion.addTab(contributionTab, messageHelper.getMessage("document.accordion.contribution"), VaadinIcons.CHEVRON_RIGHT);
            contributionTab.populateContributionsData(allContributions);
        }
    ...
    public void searchBarClosed() {
        legalTextComponent.closeSearchBarComponent();
    }

    public void enableExportPackage(boolean enableExportPackage) { legalTextActionMenuBar.setExportPackageVisible(enableExportPackage); }
