...
public class UserSearchComponent extends InputListeningComboBox {
    ...
    public UserSearchComponent(MessageHelper messageHelper, EventBus eventBus) {
        this.messageHelper = messageHelper;
        this.eventBus = eventBus;
        init();
    }

    public UserSearchComponent(String searchContext, MessageHelper messageHelper, EventBus eventBus) {
        this.messageHelper = messageHelper;
        this.eventBus = eventBus;
    ...
    void updateUserDropDown(SearchUserResponse event) {
        ...
        if (validateProposedUsers(users)) {
            List<UserVO> processedUsers = handleSameUserForDistinctEntities(users);
            processedUsers.forEach(getContainerDataSource()::addItem);
        }
    }

    @Subscribe
    void updateSearchContext(SearchContextSelectionEvent searchContextSelectionEvent){
        searchContext = searchContextSelectionEvent.getContext();
        LOG.debug("Search context set as "+searchContext);
    }

    private List<UserVO> handleSameUserForDistinctEntities(List<UserVO> users) {
        return users.stream()
                .flatMap(user -> user.getEntities().stream()
                        .map(entity -> new UserVO(user, entity)))
                .collect(Collectors.toList());
    }

    public void setExistingUsers(List<UserVO> existingUsers) {
        this.existingUsers = existingUsers;
    }

    private void init() {
        setItemCaptionMode(AbstractSelect.ItemCaptionMode.PROPERTY);
        setItemCaptionPropertyId("nameWithOrg"); // field of User bean
    ...
    private void handleUserInputChange(String userInput) {
        ...
        if ((userInput != null) && (userInput.length() >= MIN_INPUT_LENGTH)) {
            ...
            if(searchContext != null){
                eventBus.post(new SearchUserInContextEvent(userInput, searchContext));
            }else{
