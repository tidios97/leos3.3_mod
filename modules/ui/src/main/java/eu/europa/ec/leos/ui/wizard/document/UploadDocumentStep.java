...
class UploadDocumentStep extends CustomComponent implements WizardStep, Upload.StartedListener,
    ...
    private static final Logger LOG = LoggerFactory.getLogger(UploadDocumentStep.class);
    ...
    UploadDocumentStep(DocumentVO document, MessageHelper messageHelper, EventBus eventBus) {
    ...
    public void uploadSucceeded(final Upload.SucceededEvent event) {
        ...
        postProcessingProposal(document);
    ...
    private void postProcessingProposal(DocumentVO document) {
        eventBus.post(new PostProcessingDocumentEvent(document));
    }
